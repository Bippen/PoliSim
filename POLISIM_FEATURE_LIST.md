...
**EN-3b — The reservoir dispatch.** ⏳ Opened 2026-09-11 by ruling (`COMPLETED.md` §462): the energy market clears Sweden's zones at a WATER VALUE proxied by Germany's and Poland's block prices - a convention, stated in `EnergyMarket` - and runs every country's hydro at its 2023 block levels; the spec-let (S2) holds that hydro is the one storage this game must not abstract away, and Sweden and Italy are hydro systems whose price formation is a reservoir decision - the marginal value of stored water against the season's inflow, the reservoir's level and the prices it can wait for. Evidence already on the record: the proxy prices Sweden above its own 2023 retail energy component (§464's fitted supply margin for Sweden is negative). What it takes: a reservoir state per hydro system (inflow by block from the 2023 levels, a storage bound sourced - Svk/Ei publish Sweden's reservoir fill; Terna Italy's), the water value solved as the shadow price that empties the year's inflow across the blocks at maximum value, and the zonal clearing on it; its own BASELINE where it moves the power figure (Italy's dispatch). Sized one to two sessions; after EN-4.

**EN-4 — The energy track's stage 4, the fiscal layer.** ⏳ `POLISIM_ENERGY_SPECLET.md` §7 row 4 (S7, then S8 on its own pass): the two ledgers as one structure - every energy policy a system-cost entry and an incidence vector (households, industry, taxpayers, generators, the state); the retail stack wholesale + network + policy + tax seeded from and checked against Eurostat's price components (`ENERGY_LAYER_SPINE.md` §6); support schemes as appended `SpendingCategory` members indexed by P5-B2 with `SpendingDrivers`; the Energy sector's row derived (system cost ÷ nominal GDP) and `EnergySpendingChange` retired; **then S8 alone, BASELINE on six:** the retail price into `Inflation` through the sourced HICP weight (§7 of the spine). 2 + 1 sessions. Opened 2026-09-11 by stage 3's landing (§460) - **the residue row a session could start.**
