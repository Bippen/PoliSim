...
| `POLISIM_ENERGY_SPECLET.md` | stage 1 of the energy track: the collision map against `EconomyState` and the environment family, the load-block and zone form, the sourcing bill with every series named, the sizing, the strikeable design (S1–S15), awaiting Elias's ruling (E-34) | frozen until ruled |
...
| **E-34** (`ERRANDS.md`, §451) | the energy spec-let - `POLISIM_ENERGY_SPECLET.md`, stage 1 of `ENERGY_SYSTEM_ADAPTED.md`'s eight: fifteen strikeable lines and one recommendation; plus the ENTSO-E account and the source docx's whereabouts | the energy track's stage 2 (the physical layer, 3 sessions of 14–17) - **no energy code before the ruling** |
