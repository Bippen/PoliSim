...
| **FT-8** (§397) | the natural rate reads the labour force - the NAIRU half of FT-6, ruled 2026-09-08 and sheeted with its seat, its cost and its source | **the next pass's one family** - Okun, the Phillips curve and the Taylor gap term re-anchored for six; the year-300 inflation table and the gap before and after |

Nothing else is open. The strikeable decisions of this run are marked in §380 (HOLDS / RECONSIDER / NEEDS ELIAS); the fix-track table carries FT-1..FT-8 and RF-1..RF-2 each with its state (FT-8 the next pass's, by ruling §397); the shelf's last cycle is §380.
