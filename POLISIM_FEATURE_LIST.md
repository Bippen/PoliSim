...
**EN-3 — The energy track's stage 3, the market layer.** ⏳ `POLISIM_ENERGY_SPECLET.md` §7 row 3 (S5, S6): merit order per block per zone over variable cost (fuel + the ETS price + the carbon tax + O&M), transfer-constrained on the six links, a scarcity term, negative prices, congestion rent; the inputs are on `ENERGY_LAYER_SPINE.md` §5 and the fleet of stage 2 (§457); 2–3 sessions; its own BASELINE family (the carbon tax's mechanism on power moves from the family's elasticity to dispatch). EN-4..EN-8 follow in the spec-let's order. Opened 2026-09-10 by stage 2's landing - **the first residue row a session could start since the shelf read zero; the count reads it.**
...
| `ENERGY_LAYER_PREMISE.md` | GENERATED by `EnergyLayerDump` (§457): the physical layer at its seed - the six fleets with the definition named, the ten load folds, Sweden's zones and links with the flows, the single-book decomposition per country | regenerated |
| `EnergyData/` (six CSVs + `Tools/energy_prep.pl`) | the energy layer's derived data files - generated by the prep script from the sources the spine lists, read by `EnergyCatalogGenerator` into `EnergyLayerData`, held by `GeneratedCatalogCheck` | regenerated when a source lands or changes |
