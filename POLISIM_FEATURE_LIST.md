...
- **PF-5 — four bill call-to-action buttons still wear the retired pack's face.** THREE OF FOUR CLOSED 2026-09-22, `COMPLETED.md` §564 (the sitting pass's Track 1): the Labor Market, Trade and Emergency Drawdown bills draw board 6a's one-width button beside their sentence through `DrawSentenceAction`, the primitive every call to action now shares. OPEN for the fourth: the Budget bill's full-width *Introduce Budget Bill* in the Budget's summary column - a measured-height column the primitive's row does not fit as it stands - goes with the pass's Track 4 CTA-face item. Found 2026-09-21 (§550) while fixing Design's sighting on the Energy tab's.
...
- **CL-5 — THE LIVE GAME DOES NOT RUN THE CALENDAR THE PLAY PROTOCOL NAMES. RULING-FIRST, Elias's.** Found 2026-09-21 (§558). The protocol, its staging and its check read the static Swedish calendar; the live day path reads the next election turn's boundary, so on the protocol's own save the run-up begins more than three years after the day the protocol says it opens on. Either the live game runs a country's own dates (and then what the four-turn cycle means for the other five), or the protocol opens where the game's run-up does. E-39 is blocked on it; the staging prints both calendars as a warning on every run.
...
| `SEND_PACKAGE.md` | the current paste-and-glance package for Design | regenerated |
| `ELECTIONS_CAMPAIGN_SPEC.md` | **REFERENCE** - the 44-section elections specification cited by §; says so at its head | frozen |
| `POLISIM_TAX_SPECLET.md` | C-C12's spec-let: the tax instruments and the sourcing bill - RULED 2026-08-31 (D-3), its sheets §474 | F4-1, F4-2 and F4-4 built; F4-3 next |
| `ENERGY_SYSTEM_ADAPTED.md` | **REFERENCE-AND-PLAN** - the external energy concept adapted to this game: the collisions, what is kept, the eight stages; its source docx is external reference material, not on disk (§451) | frozen; its provenance line changes if the docx lands |
| `POLISIM_ENERGY_SPECLET.md` | stage 1 of the energy track: the collision map against `EconomyState` and the environment family, the load-block and zone form, the sourcing bill with every series named, the sizing, the design S1–S15 - **RULED 2026-09-10 as recommended (E-34, §454)**; Design's map prototype recorded in its §3.2 as evidence (§452) | frozen; its status head carries the ruling |
| `ENERGY_LAYER_PREMISE.md` | GENERATED by `EnergyLayerDump` (§457, §459): the physical layer at its seed - the six fleets with the definition named, the ten load folds, Sweden's zones and links with the flows, the single-book decomposition per country - and the market at its seed: every block's clearing, the adders, Sweden's zones against the NTCs, snitt 4's answer | regenerated |
...
| `ENERGY_LAYER_SPINE.md` | stage 2's data spine (§455): generation, fleet, the load folded into the three blocks for six countries and SE1–SE4, Sweden's zones and links from the authorities, fuel and carbon prices, the retail stack, the HICP weights, the electrification stock, the EDGAR residual's first reading, the two BILLED rows, every file by digest | grows by section as stages 3–4 source their rows |
| `SOCIETY_STATS_CATALOG.md` | P5-C1: Elias's metric list mapped to the six countries' sources | grows with families |
| `HEALTH_FAMILY_SPINE.md`, `EDUCATION_FAMILY_SPINE.md`, `INFRASTRUCTURE_FAMILY_SPINE.md`, `ENVIRONMENT_FAMILY_SPINE.md`, `IMMIGRATION_POVERTY_FAMILY_SPINE.md` | the five families' data spines: seeds with their sources, couplings, fetch outcomes, feedback passes | grow by section |
| `BUDGET_PREMISE.md` | GENERATED by `BudgetPremiseDump` (P5-B1): what a line and a tax are, probed | regenerated |
| `POTENTIAL_PREMISE.md` | GENERATED by `PotentialPremiseDump`: the premise of potential output, measured before and after P5-B7, re-measured after §394 (`potential03`) and §522 (`potential04`) | regenerated |
| `PLAY_PROTOCOL.md` | CL-3's play protocol (§523): the seed stated (777; the 2022 prior until K-1), the four saves and what each opens on, the play's five steps, the sheet, the record | frozen until a play changes it |
| `PLAY_SHEET.md` | GENERATED by `PlaySheetGenerator` (§523): §346's twenty entries with each constant read off the code by reflection and its one thing to look for; the last column is Elias's and survives regeneration; `PlaySheetCheck` holds the rest | regenerated; the last column by hand |
