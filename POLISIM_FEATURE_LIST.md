...
**· ⏳ F2e - the connection queue's capacity. OPEN.** §544 measured it: the queue accepts an order of any size, and Germany's ministry opens with a single year's order nearly the size of the country's whole wind fleet. A yearly capacity on the queue, per technology, SOURCED - the EEG's own tender volumes are the candidate §544 named for Germany; the other five need theirs or a BILLED row. The player's orders pass through the same queue, so the cap is the player's too, with its refusal as a sentence on the line like the others. Then, and after FT-10's three families, the flip is its own BASELINE family.
...
- **The review's unexecuted proposals** (`COMPLETED.md` §190, its §B.4): **P3** the session start-up
  ...
  **P5** records generated from git · **P6b** per-country trajectories (P6a, the artifact check made
  ...
  F2–F6.
- **FT-10 — the energy layer's price indices run away** (found by §538's diff, present in every baseline since EN-4; measured `COMPLETED.md` §541). **RULED 2026-09-21: three families in a fixed order, one per pass.** **✅ P-A LANDED, §545 (`e0f5064`), baseline `traj_p6pa`:** the levy scale reads the support line's move as a SHARE of its own path, times K, the seed's line over the seed's levy - the growth index no longer stands on one side of the ratio, Poland's scale holds level for a thousand years where it had compounded, and the subsidy dial's levy cut, which had the same defect, is steady; the sentinel's recorded cells were lowered in the same commit (the record has the table), each now carrying its cause. **⏳ P-B NEXT (the next pass's family):** Sweden's water value in the seed's prices - each neighbour's price deflated by its own price level before the weighting, carried by Sweden's level once; Sweden alone moves, and Sweden's cells wait on it. **⏳ P-A′ THIRD (ruled as a ruling on EN-4):** the levy rule reads the line's SUPPORT SHARE only, not the whole energy line - France's and Sweden's K are the whole line over a small levy, bounded since P-A and not right; France's cells wait on it, and the share needs a source per country. What stands after all three is the rule's own bounded answer to a ministry's cut (Poland, Italy), recorded as such.
...
- **PF-4 — *Retail intervention*'s range captions promise a levy floor the dial cannot reach in two countries, and reach it early in the other two.** OPEN, found 2026-09-21 by §545's review, in §542's captions (mine, the same day). The dial's whole cost is a fixed share of GDP and the seed levies differ by two orders of magnitude, so *"On the house - the levy at its floor"* and *"Lavish - the levy nears nothing"* are false for Italy and Poland, where the full dial leaves most of the levy standing, and late for Sweden and France, where the levy is gone a few points above neutral and the upper bands describe a levy that no longer exists. `RangeCaptionCheck` holds the SIGN, on one country; the ten lines are one catalog for four stacks. The fix is its own item: bands per country read off the model, or lines that promise a direction and not a floor. The review's table is in `Reviews/`.
