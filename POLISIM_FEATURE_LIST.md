...
**EN-3b — The reservoir dispatch.** ⏳ Opened 2026-09-11 by ruling (`COMPLETED.md` §462): the energy market clears Sweden's zones at a WATER VALUE proxied by Germany's and Poland's block prices - a convention, stated in `EnergyMarket` - and runs every country's hydro at its 2023 block levels; the spec-let (S2) holds that hydro is the one storage this game must not abstract away, and Sweden and Italy are hydro systems whose price formation is a reservoir decision - the marginal value of stored water against the season's inflow, the reservoir's level and the prices it can wait for. Evidence already on the record: the proxy prices Sweden above its own 2023 retail energy component (§463's fitted supply margins: households −0.0068, non-households −0.0188 USD per kWh in the book's dollars). What it takes: a reservoir state per hydro system (inflow by block from the 2023 levels, a storage bound sourced - Svk/Ei publish Sweden's reservoir fill; Terna Italy's), the water value solved as the shadow price that empties the year's inflow across the blocks at maximum value, and the zonal clearing on it; its own BASELINE where it moves the power figure (Italy's dispatch). Sized one to two sessions; after EN-4.
...
**EN-4c — The carbon tax's unit (ruling-first).** ⏳ Opened 2026-09-11 (`COMPLETED.md` §461, §463): the book prices a carbon-tax point as a share of GDP on the emissions base (`TaxBases`, P5-B3 - revenue = rate ÷ 100 × the seed share of GDP × the emissions ratio × the price level), while the dispatch prices it as ONE UNIT OF THE COUNTRY'S CURRENCY PER TONNE (`EnergyMarket.CarbonTaxPointPerTonne`, a stated convention) - two readings of one line, and the fiscal layer books the power sector's payment under the second while the budget's revenue follows the first. Sweden's implemented 30 reads as 30 SEK per tonne against a statutory 1 330; Poland's twenty points are €4.40. The ruling to take: what a point IS (currency per tonne, with the rate range re-scaled; or the share-of-GDP reading kept and the dispatch's bridge re-derived from the book's own revenue per tonne at the seed) - and the ETS proceeds' place in the book (a system-cost line today, unbooked as revenue). Nothing built until ruled; the dump prints both readings.

**EN-5 — Stage 4's second pass, the HICP pass-through (S8).** ⏳ Opened 2026-09-11 by EN-4's landing: the household retail price the ledger writes enters `Inflation` through the sourced HICP weight - electricity CP0451 per mille DE 29.63 · FR 28.21 · IT 33.15 · PL 23.87 · SE 70.50 (`ENERGY_LAYER_SPINE.md` §7; the USA's BLS relative importance BILLED) - as its own BASELINE on six, explained per country; the expectations anchoring (§386) reads the headline it lands in, so the pass names what a Swedish electricity year does to the Riksbank's path. One session, after EN-4c's ruling where the price's tax half depends on it.
