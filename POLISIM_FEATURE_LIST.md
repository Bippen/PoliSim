...
| **FT-9** (§400) | Okun's daily form reads the boundary's GDP jump as growth the year never printed - the remaining tenth of the NAIRU gap, measured to the hundredth for six | the boundary jump decomposed by step, then the form; the unemployment gap reading zero on average |
...
Nothing else is open. The strikeable decisions of this run are marked in §380 (HOLDS / RECONSIDER / NEEDS ELIAS); the fix-track table carries FT-1..FT-8 and RF-1..RF-2 each with its state (FT-8 landed §398); the shelf's last cycle is §380.
