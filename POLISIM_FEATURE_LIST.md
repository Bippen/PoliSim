...
**EN-4d — Which tonnes the carbon tax covers (ruling-first).** ⏳ Opened 2026-09-11 (`COMPLETED.md` §464): the game's carbon tax is levied on the model's two sectors - power and transport CO₂ (§349) - at the statutory rate, while the statutes exempt ETS installations (the power fleets pay the ETS the merit order already carries) and tax heating fuels the model does not carry. Germany is the measure: 30 EUR on the book's 318 Mt is 9.56 bn EUR against a real BEHG take near 10.7 bn on heating and transport - the magnitudes meet because power in and heating out roughly offset, the meaning does not, and the ledger books a power-sector carbon payment the statute does not levy. The ruling to take: a per-country coverage of the two sectors (Sweden, Germany, France: transport in, power out at the seed) and a lever to extend the tax to power - which would also decide whether the EN-3 mechanism (the tax reaching the dispatch) is the player's to switch on. Nothing built until ruled.

**EN-5 — Stage 4's second pass, the HICP pass-through (S8).** ⏳ Opened 2026-09-11 by EN-4's landing: the household retail price the ledger writes enters `Inflation` through the sourced HICP weight - electricity CP0451 per mille DE 29.63 · FR 28.21 · IT 33.15 · PL 23.87 · SE 70.50 (`ENERGY_LAYER_SPINE.md` §7; the USA's BLS relative importance BILLED) - as its own BASELINE on six, explained per country; the expectations anchoring (§386) reads the headline it lands in, so the pass names what a Swedish electricity year does to the Riksbank's path. One session; EN-4c ruled the tax's unit on 2026-09-11, so nothing blocks it.
