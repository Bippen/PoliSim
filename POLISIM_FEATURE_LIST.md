...
| **FT-7** (§383) | the jobs lag - labour supply arriving before employment, so unemployment reads participation and output follows with a lag; ruling-first, its seat and source named | FT-5's response to the labour tax (sheeted behind it) and FT-6's NAIRU half |
| **The AI finance ministry, two strikeable choices** (§387, §388) | built as the page recommends; what a ruling could strike: the US sequester sized at the EU benchmark's 0.5 % of GDP (borrowed, since the statute sizes it to a cap's breach), and the 2024 safeguards read as adjustments of the headline balance where the law reads the structural one | the AI countries' debt paths, which the player sees; nothing waits on the choice |
