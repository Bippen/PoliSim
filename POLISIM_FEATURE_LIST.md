...
**EN-3c — The market's seed prices against the exchange (evidence, ruling-first).** ⏳ Opened 2026-09-11 (`COMPLETED.md` §466): the same fold that measured Sweden measured the proxy's two markets - the exchange's 2023 blocks for Germany 72.8 / 93.5 / 131.1 €/MWh (load-weighted 98.3) against the market's seed 82 / 90 / 101, Poland 80.2 / 111.0 / 148.1 (115.1) against 68 / 82 / 97. The adders calibrate the fossil SHARES, not the prices, and the peak blocks run a quarter low; whether a second calibration - a scarcity or a price-level fit per country - is wanted, and against which figure (the exchange's day-ahead or the dispatch's own cost), is a ruling. Nothing built until ruled; the dump prints both. **MEASURED AND PROPOSED 2026-09-12 (`COMPLETED.md` §470):** the gap is a SHAPE - the mid block within three euro of Germany's exchange, the base above it, the peak a quarter to a third low in both markets (load-weighted −7.0 €/MWh Germany, −32.0 Poland); a level calibration moves nothing reachable at the seed but the wholesale-against-margin split (the stack's total is Eurostat's and the fitted margin closes it) and the ledger's unmodelled inframarginal rent, and no stage reads the wholesale level yet. Two forms costed - a price adder per block (six to fifteen FITTED parameters, nil response) or a scarcity term per country (one each, but it moves gate 7's reference and every peak figure). **Recommended: not now** - re-read when stage 5's fleet reads its rent, with the three missing folds (EPEX FR, GME, an EIA hub) fetched first. Still open, ruling-first. ⏳
...
**EN-4e — Sweden's statutory indexation of the carbon tax rate (a known divergence, ruling-first).** ⏳ Opened 2026-09-12 (`COMPLETED.md` §468): lag (1994:1776) om skatt på energi, 2 kap. 1 b § (as amended by lag (2026:372); `ENERGY_LAYER_SPINE.md` §14) recalculates the koldioxidskatt every year by a *jämförelsetal* - the general price level in June over June of the year before, four decimals; the government fixes the coming year's amounts before the end of November - so the real rate holds by construction. The model's 1 330 SEK is nominal-fixed and erodes as an excise does (`CarbonTaxUnitDiagnostic` (6)); the divergence is Sweden's alone (Germany's BEHG is a legislated path, France's composante carbone frozen since 2018). The ruling to take: carry the statute (the rate recalculated at each boundary by the price level's ratio - a dozen lines on the standing boundary, moving Sweden's carbon revenue, its transport coupling and the political scale's reading of an indexed rise) or stay nominal-fixed and say so. Nothing built until ruled.
...
Nothing else is open. The strikeable decisions of this run are marked in §380 (HOLDS / RECONSIDER / NEEDS ELIAS); the fix-track table carries FT-1..FT-9 and RF-1..RF-2 each with its state (FT-8 landed §398, FT-9 landed §404); the shelf's last cycle is §469 (2026-09-12; §436 and §380 before it).
