...
- **S6. ✅ BUILT 2026-09-15 as F4-3 (`COMPLETED.md` §514): the line seeded at the municipal layer's 32.38, the state layer its second layer, the base re-derived, the bridge re-solved; seeded revenue held by D-16's anchors to the base row's rounding, the family running through FT-5's participation term.** Sweden's blended 52 is retired at this build, replaced by kommunal + statlig as two
  ...
  family must be explained per country under the full sim-math bar. **DS-2d (§474): one line, two layers - S5's one revenue figure per `TaxType` holds and S6's two real instruments hold; "both, not either" (F4-3).** **F4-5 (§517) generalises S6's answer:** where a statute names one rate every unit of taxed income pays, the lever is seeded at that rate (Sweden's 32.38); where it does not, the lever is seeded at the schedule's own average rate on the income it taxes, which is what the revenue engine requires and which returns Sweden's own figure to within four tenths of a per cent.
