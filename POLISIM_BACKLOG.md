...
| **S-23** | ⚠ **`DeadStateCheck` claims a class it does not catch.** Its own doc says *"a field written and never read"* is caught; it is not — the detector counts occurrences, so a declaration plus a write is two and passes whatever reads it. Named in the check's header 2026-09-01 and the write-only family cleared by hand, but **distinguishing a read from a write needs more than a regex** and the limitation stands | CODE | `COMPLETED.md` §138 |
...
| **S-29** | ⚠ **The party-ink constraint is now a DRAW-SITE rule, and nothing asserts it.** D9 row 5 ruled the derived 8.7° hue floor the wrong constraint — it keeps two AREA accents apart, and party inks never sit in that company — so `PartyInkHarness`'s six PENDs are **answered, not fixed**, and their measurements stay printed. What binds instead is structural: **(1) party ink is never drawn adjacent to an area accent; (3) a party swatch forced into chrome (a live-election status dot) draws in the NEUTRAL STATUS INK, not the party's.** ⚠ **A hue harness cannot see either** — both are about where a colour is drawn, not what it is. Sized as its own row: a check that enumerates the party-ink call sites (today three, all in `HemicycleRenderer`) and fails when one sits inside a chrome surface. **Prove it both directions**, as the fifth sweep did | CODE | `COMPLETED.md` §146 |
