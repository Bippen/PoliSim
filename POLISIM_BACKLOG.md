...
| C-C9 | P-G1 — the shadow baseline | 9 (deep half) | ⏳ **IN PROGRESS — the premise is MEASURED and THE PROOF GATE IS PASSED (`COMPLETED.md` §§103–104); the shadow does not yet reach a screen.** `ShadowBaseline` wraps every shadow turn in a save/swap/restore of the generator state, restoring in a `finally`. Gate: counters unchanged · ⚠ **two real games, one with a shadow beside it and one without, BYTE-IDENTICAL over 8 turns on every `EconomyState` field** (the assertion that binds — a counter check cannot substitute) · the shadow equals a plain no-policy world · a shadow turn that THREW left the real state untouched. Cost shipped as measured (~148 ms the pair), lazy computation named as the fallback and not built. Remaining: the shadow history, the overlay, and the divergence-equals-batch-diff proof. ~~the premise is MEASURED (`COMPLETED.md` §103), the build is not started.** ⚠ **A shadow turn consumes 41 real draws** (`PublicationRevision` 18, `SovereignWealth` 16, `Event` 7), so a naive shadow would make merely LOOKING at the counterfactual change the real game future — a determinism break inside a passive-read feature. **The build starts with a `CaptureDrawCounts`/`RestoreState` wrapper and an assertion that the real counts are identical across a shadow turn** (C-C2-shaped evidence, binding where the change lives). Cost measured and stated: real turn **50.4 ms**, shadow turn **97.4 ms**, pair **~148 ms** — nearly a tripling, not a doubling. Remaining: the wrapper + assertion, the shadow history, the overlay, and the two done-when proofs | — |
