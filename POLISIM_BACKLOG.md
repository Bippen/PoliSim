...
| C-R4 | The rail cell and the win/lose rule | ⏹ **STOPPED AND REPORTED 2026-08-31 (`COMPLETED.md` §119), with the one reason named exactly — which is this row's own done-when.** `GameController.cs:2090` says it about itself: **"No player path sets `_campaignScreen`."** Every Track E screen draws from a `CampaignSnapshot` and **the only thing that builds one is `UiScreenshotDriver`**. Two further blockers, both someone else's: ⚠ **the war chest is UNRULED** (C-D2 proposed four options and applied none — the plan's own fallback is that the cell then stays absent and says why), and ⚠ **the win/lose rule is a RULING, not a refactor** — "game over only on leaving office" changes what `ScenarioEvaluator` means, and this track's pre-ruling says anything that would move a baseline stops and reports | **ELIAS** then CODE | RULING-BLOCKED | C-R3, C-D2 |
