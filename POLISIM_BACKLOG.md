...
| C-C5 | P-C1 — national currency display | 5 | SAFE | ⚠ **RE-ORDERED: runs AFTER C-C6** (R-N1 fork, logged at `COMPLETED.md` §98). The seeds are USD; printing "620 miljarder kr" against a dollar figure would be false by ~10.5×, so the display half depends on the basis half. **Blocked further on a sourced FX rate** — see C-C6 |
