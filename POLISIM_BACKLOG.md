...
| S-18 | **The player's interest-rate lever is DEAD in every country, confirmed from outside C-C7's own evidence.** `CurrencySystem.ApplyInterestRateChanges` (`:45-50`) routes any country with `CurrentFedChair != null` to the bank's own rule and never reads `PolicyDecision.InterestRateChange`; since C-C7 seeded governors for Sweden and Poland, all six qualify. Intended, and the screens say so — but **`PolicyDecision.InterestRateChange` and its long doc comment are still there**, and the next reader will believe them. Found when a diagnostic built on that lever reported itself untested rather than passing | CODE | `COMPLETED.md` §105 |
