...
{
    /// <summary>
    /// FT-8 (2026-09-08, §398): the gap and the NAIRU column read the EFFECTIVE natural rate - seed-plus-laws plus the labour force's demographic shift - and the MEAN
    /// line prints the natural rate at the start and the end. The before/after of §398's headline is infldrift3 (before) against infldrift4 (after).
            ...
            try
            {
                ...
                for (int year = 1; year <= Turns; year++)
                {
                    ...
                    foreach (Country c in world.Countries)
                    {
                        ...
                        float gap = s.Unemployment - c.EffectiveNaturalUnemploymentRate;   // FT-8 (§398): the gap the core reads
                        ...
                        if (year % 25 == 0 || year == 1)
                        {
                                                        sb.Append($"INFLDRIFT: {year,4}  {c.Id,-8} {s.Unemployment,6:F2} {c.EffectiveNaturalUnemploymentRate,6:F2} {gap,7:F3} {growthGap,10:F3} {s.Inflation,7:F2} {s.InflationExpectations,7:F2} {(c.CurrencyZone?.InterestRate ?? -999f),7:F2} {TaylorRule.GetSuggestedInterestRate(c),7:F2}\n");
                        }
                    }
                }
                int n = Turns - 5;
                foreach (Country c in world.Countries)
                {
                    sb.Append($"INFLDRIFT: MEAN {c.Id,-8} gap {gapSum[c.Id] / n,+8:F4} (below NAIRU {100.0 * belowCount[c.Id] / n:F0} % of turns; the natural rate {c.NaturalUnemploymentRate:F2} → {c.EffectiveNaturalUnemploymentRate:F2} at year {Turns})  growth−potential {growthGapSum[c.Id] / n,+8:F4}  inflation−expectations {wedgeSum[c.Id] / n,+8:F4}  (the Phillips slope 0.3 × −gap → {-PhillipsSlope * gapSum[c.Id] / n,+8:F4}); the nudges inferred: reversion 0.7 × gap + Okun 0.5 × growth gap = {0.7 * gapSum[c.Id] / n + 0.5 * growthGapSum[c.Id] / n,+8:F4} per turn\n");
                }
