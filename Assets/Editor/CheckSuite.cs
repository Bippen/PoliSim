...
namespace PoliSim.EditorTools
{
    ...
    public static class CheckExit
    {
        ...
        public static int Collect(Action check)
        {
            ...
            // the Editor mid-suite. `D18Inventory` runs five coverage checks inside its own run, so the
            // previous state is saved and restored rather than assumed.
            ...
            finally
            {
                ...
                // An inner run reports its code through the return value; the outer tally is the outer
                // check's own and must come back exactly as it was.
                _worst = wasCollecting ? outerWorst : _worst;
            }
        }
    }
    ...
    public static class CheckSuite
    {
        ...
        private static readonly (string Name, Action Run)[] Suite =
        {
            ...
            // describes. ⚠ It is the only check here that reports on a document sent to another party,
            // and the failure it exists for is the one section 0 of that document is living proof of: a
            ...
            ("ChamberVerdictCacheCheck", ChamberVerdictCacheCheck.Run),   // PF-1 (2026-09-17, §525): the screens' cached chamber answers are the model's own, through a draft, a seat, a party and a government change (§642)
            ...
            ("EarnedIncomeCreditDiagnostic", EarnedIncomeCreditDiagnostic.Run),   // P6-E1 (2026-09-17, §534): jobbskatteavdraget to the krona against SKV 433's examples; held off the yield until ruled
        ...
        private static readonly (string Name, Action Run)[] Simulation = new (string Name, Action Run)[]
        {
                ...
                ("AppliedCostIdiomDiagnostic", AppliedCostIdiomDiagnostic.Run),   // §497 (2026-09-14): the dial costs' trackers ride their line's index and cross the preview clone
                ...
                ("GovernmentConsumptionDiagnostic", GovernmentConsumptionDiagnostic.Run),   // T-3 form A (2026-09-15, §507): the identity's G the sourced share at the seed, the day's call site handing it, the lines carrying it, the book unscaled
                ...
                ("LabourInputWindowDiagnostic", LabourInputWindowDiagnostic.Run),   // PN-3 (2026-09-16, §522): potential's labour input is the pyramid's labour force (the 15+ population at the state's rate, employed) and not the 20–64 window - the table's identity, the 15+ form's identity and the input's, at the seed and after 25 years; the window printed as the deviation retired; the wage bill's same seam measured (PN-3b)
                ...
                ("EnergyMinistryDiagnostic", EnergyMinistryDiagnostic.Run),   // P6-F2d (2026-09-21, §544): the AI energy ministry HELD and driven for forty years - the mandates answered in their own years, the veto, what the orders do to the clearing
                ...
                ("PensionPaymentDiagnostic", PensionPaymentDiagnostic.Run),   // PN-2/PN-4 (2026-09-16, §518/§519): the pension line over the cohorts the statute retires, and the seed against its source SAME YEAR - the provenance guard (a seed that misses its COFOG share by a hundredth of a point), the all-types bound, the perimeter read beside the verdict, §518's two figures decomposed. Ran standalone at §518; on the bar from §519
                ("PensionAgeDiagnostic", PensionAgeDiagnostic.Run),   // PN-1 (2026-09-13): the pension age's statute layer against its CSV - the rule kinds, the indexed class of two, the dated paths (Sweden's six years out)
                ("PensionParticipationDiagnostic", PensionParticipationDiagnostic.Run),   // PN-1's other half (2026-09-23, s596): the participation response per country - zero at the seed, the statute's path, the dial both ways, the line's saving unchanged
                ("TaxScheduleDiagnostic", TaxScheduleDiagnostic.Run),   // F4-2 (2026-09-13): the income tax as its statute's shape - the incomes in the statute's currency, the exempt bands untouched by the shift, the ratio one at the seed, the drag at no policy
                ...
                ("EnergyPassThroughDiagnostic", EnergyPassThroughDiagnostic.Run),   // EN-5 (2026-09-11): the retail electricity price reaches Inflation through the sourced index weight - nothing passes at no policy, Poland's raise prints weight × the real change and expectations look through it, Germany's raise reaches Sweden's print through the water value, B6
                ("ReservoirDispatchDiagnostic", ReservoirDispatchDiagnostic.Run),   // EN-3b (2026-09-11): Sweden's zones at the exchange's 2023 prices with their measured coupling to the continent, the reservoirs carried and a deficit raising the water value, Italy's and Germany's shiftable hydro restoring the seed spread under a policy, B6, the Swedish margins turned
