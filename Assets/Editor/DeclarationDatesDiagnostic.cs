...
namespace PoliSim.EditorTools
{
    ...
    /// lifted 8 September. Every fact's dates are ordered and no two facts of one party and kind overlap.
    /// </summary>
    public static class DeclarationDatesDiagnostic
    {
        public static void Run()
        {
            ...
            try
            {
                ...
                string Rules(List<InOrAgainst> rules) { var k = new List<string>(); foreach (InOrAgainst r in rules) { k.Add(parties[r.Party].Abbrev); } k.Sort(StringComparer.Ordinal); return string.Join(",", k); }
                ...
                Check(Rules(DeclaredRedLines.InOrAgainstAt(CountryId.Sweden, parties, day2026)) == Rules(DeclaredRedLines.InOrAgainstFor(CountryId.Sweden, parties, ElectionVintage.Sweden2026)) && Rules(DeclaredRedLines.InOrAgainstAt(CountryId.Sweden, parties, day2026)) == "V", "2026: the in-or-against rules agree (V)");
                ...
                Check(Has(start, "C", "SD", false, true) && Has(start, "M", "SD", false, false) && Has(start, "KD", "SD", false, false) && Has(start, "L", "SD", false, false)
                    ...
                    && DeclaredRedLines.InOrAgainstAt(CountryId.Sweden, parties, WorldClock.StartDate(CountryId.Sweden)).Count == 0,
                    F("{0:yyyy-MM-dd} (the start): the 2022 lines and the carried candidacy pair, no C>V, no V rule - [{1}]", WorldClock.StartDate(CountryId.Sweden), Shape(start)));
                ...
                Check(DeclaredRedLines.InOrAgainstAt(CountryId.Sweden, parties, new DateTime(2026, 4, 17)).Count == 0 && DeclaredRedLines.InOrAgainstAt(CountryId.Sweden, parties, new DateTime(2026, 4, 18)).Count == 1, "2026-04-18: V's rule takes effect (the decision's day, not the PDF's)");
