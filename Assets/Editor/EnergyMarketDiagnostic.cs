...
namespace PoliSim.EditorTools
{
    ...
    /// the adders printed. (3) THE MECHANISM: Poland with its fleet's ETS price stepped twenty euro per tonne (a probe on that fleet alone - since
    /// EN-4d, §467, the national carbon tax does not reach ETS-covered plant; it reaches transport, which the step leaves alone), ten years, against
    /// untouched - power CO₂ per head LOWER, coal's share of the fossil total LOWER, gas's HIGHER, the peak price HIGHER; the transport figure
    /// printed beside, not moved by the fleet's price. (4) SWEDEN'S ZONES: the flows the coincident-hour balances require on the three snitt
    ...
    /// residual exceeds the dependable capacity. (6) B6: at a doubled price level the whole marginal cost doubles - fuel, the ETS and the adders
    /// alike (the national carbon tax is not in it, EN-4d) - and an ETS step adds its euro on the emission factor, carried by the level.
    /// </summary>
    public static class EnergyMarketDiagnostic
    {
        ...
        public static void Run()
        {
            ...
            foreach (Country c in world.Countries)
            {
                ...
                float written = EnergyMarket.PowerCo2PerHead(c);
            ...
            sb.Append(F("\n    3. THE MECHANISM: Poland, its fleet's ETS price stepped {0:F0} EUR/t from the top of year 1 (a probe on that fleet alone; EN-4d: the national carbon tax does not reach ETS-covered plant), ten years, against untouched\n", EnergyMarket.ResponseStepEtsPerT));
            double[] untouched = RunCountry(CountryId.Poland, Years, 0f);
            double[] raised = RunCountry(CountryId.Poland, Years, EnergyMarket.ResponseStepEtsPerT);
            // [0] power t/head, [1] coal share, [2] gas share, [3] peak price, [4] transport, [5] the ETS price the fleet cleared at
            if (!(raised[0] < untouched[0])) { ok = false; Debug.LogError($"ENERGY MARKET: Poland's power CO₂ did not fall under the ETS step ({raised[0]:F4} against {untouched[0]:F4})."); }
            if (!(raised[1] < untouched[1]) || !(raised[2] > untouched[2])) { ok = false; Debug.LogError($"ENERGY MARKET: the step did not move Poland from coal to gas (coal {untouched[1]:F3} → {raised[1]:F3}, gas {untouched[2]:F3} → {raised[2]:F3})."); }
            if (!(raised[3] > untouched[3])) { ok = false; Debug.LogError($"ENERGY MARKET: Poland's peak price did not rise under the step ({raised[3]:F1} against {untouched[3]:F1})."); }
            if (Math.Abs(raised[5] - untouched[5] - EnergyMarket.ResponseStepEtsPerT) > 1e-9) { ok = false; Debug.LogError($"ENERGY MARKET: the fleet cleared at an ETS price of {raised[5]:F2} against {untouched[5]:F2} - not the step."); }
            sb.Append(F("    untouched: power {0:F4} t/head, coal {1:F3} / gas {2:F3} of fossil, peak price {3:F1} at an ETS price of {4:F1}; stepped: power {5:F4}, coal {6:F3} / gas {7:F3}, peak price {8:F1} at {9:F1}; transport {10:F4} → {11:F4} ({12:+0.000;-0.000} % - the fleet's price does not reach it; what moves it is the macro path the bills changed)\n",
                untouched[0], untouched[1], untouched[2], untouched[3], untouched[5], raised[0], raised[1], raised[2], raised[3], raised[5], untouched[4], raised[4], 100.0 * (raised[4] / Math.Max(1e-9, untouched[4]) - 1.0)));
            ...
            EnergyMarket.Result rse = EnergyMarket.Clear(se);
            ...
            // (6) B6
            double mc1 = EnergyMarket.MarginalCost(CountryId.Poland, EnergyMarket.Coal, 1.0, 0.0, 0.0), mc2 = EnergyMarket.MarginalCost(CountryId.Poland, EnergyMarket.Coal, 2.0, 0.0, 0.0), mcEts = EnergyMarket.MarginalCost(CountryId.Poland, EnergyMarket.Coal, 1.0, 10.0, 0.0), mcEts2 = EnergyMarket.MarginalCost(CountryId.Poland, EnergyMarket.Coal, 2.0, 10.0, 0.0);
            if (Math.Abs(mc2 - 2 * mc1) > 1e-6) { ok = false; Debug.LogError($"ENERGY MARKET: at a doubled price level Poland's coal costs {mc2} against {mc1} - every part should double exactly."); }
            int pl = EnergyLayer.Index(CountryId.Poland);
            // EN-4d: the ETS is the fleet's carbon price, in the market's own currency - ten euro of ETS add ten euro per tonne on the emission factor, carried by the level (the national tax's zloty points, §461, are out of the merit order)
            if (Math.Abs((mcEts - mc1) - 10.0 * EnergyLayerData.EmissionFactorTPerMwh[pl][0]) > 1e-6) { ok = false; Debug.LogError("ENERGY MARKET: ten euro of ETS do not add ten euro per tonne on the emission factor."); }
            if (Math.Abs(mcEts2 - 2 * mcEts) > 1e-6) { ok = false; Debug.LogError($"ENERGY MARKET: the ETS step at a doubled price level reads {mcEts2} against twice {mcEts} - the step is nominal with nominal."); }
            sb.Append(F("\n    6. B6: Poland's coal at the seed {0:F2}, at a doubled price level {1:F2} (x2), with ten euro of ETS {2:F2} (+{3:F2} = 10 × {4:F4} t/MWh), the same step at a doubled level {5:F2} (x2)\n", mc1, mc2, mcEts, mcEts - mc1, EnergyLayerData.EmissionFactorTPerMwh[pl][0], mcEts2));
        ...
        private static double[] RunCountry(CountryId player, int years, float etsStepPerT)
        {
            SimulationRandom.Seed(777);
            EnergyMarket.ResetCalibration();
            World world = WorldFactory.CreateDefault();   // resets the probes (ResetTurnState) - the step is set after it
            // EN-4d: the probe on the player's fleet alone, from the top of year 1's turn (the ETS is set at the turn's top, as BeginTurn reads it); the national carbon tax
            // no longer reaches the fleet, so the staging the first form used - the line implemented at 5 and raised through the decision - is retired with it
            EnergyMarket.ProbeEtsRisePerT = etsStepPerT; EnergyMarket.ProbeEtsRiseOnly = player;
            ...
            try
            {
                ...
                double fossil = r.AnnualGwh[0] + r.AnnualGwh[1] + r.AnnualGwh[2];
                return new[] { c.State.PowerCo2PerCapita, fossil > 0 ? r.AnnualGwh[0] / fossil : 0, fossil > 0 ? r.AnnualGwh[1] / fossil : 0, r.Zones[0][2].Price, c.State.TransportCo2PerCapita, EnergyLayerData.EtsPerT[EnergyLayer.Index(c.Id)] + r.EtsRisePerT };
            }
