...
namespace PoliSim.EditorTools
{
    ...
    /// (2) THE COUPLING: Germany's carbon tax raised fifty dollars per tonne moves each Swedish zone by its own measured share of the proxy's move.
    ...
    /// balance is bounded by the store; at the 2023 inflow the balance stays zero. (4) THE SHIFT: Italy's carbon tax raised fifty dollars per tonne
    ...
    public static class ReservoirDispatchDiagnostic
    {
        ...
        public static void Run()
        {
            ...
            sb.Append(F("\n    2. THE COUPLING: Germany raises {0:F0} EUR/t (year 1); each Swedish zone moves by its own share of the proxy's move\n", eurRaise));
            {
                Outcome untouched = RunWorld(Years, null, 0f, 1.0);
                Outcome raised = RunWorld(Years, CountryId.Germany, eurRaise, 1.0);
                for (int b = 0; b < 3; b++)
                {
                    double proxyMove = raised.Proxy[2][b] - untouched.Proxy[2][b];   // year 2: the water value is set at the top of the turn from the standing rates
                    for (int z = 0; z < 4; z++)
                    {
                        double move = raised.ZonePrice[2][z][b] - untouched.ZonePrice[2][z][b];
                        double expected = EnergyLayer.ZoneBetaToProxy(EnergyLayer.SwedenZones[z]) * proxyMove;
                        // the two runs' Swedish price levels differ by a hair by year 2 (the seed price rides it), so the share is read to a thousandth
                        if (Math.Abs(move - expected) > 0.01 + 1e-3 * Math.Abs(expected)) { ok = false; Debug.LogError($"RESERVOIR: {EnergyLayer.SwedenZones[z]}'s {EnergyLayerData.DispatchBlocks[b]} block moved {move:F4} on a proxy move of {proxyMove:F4} - not its share {EnergyLayer.ZoneBetaToProxy(EnergyLayer.SwedenZones[z]):F2}."); }
                    }
                    sb.Append(F("    {0,-4} proxy {1:+0.00;-0.00} €/MWh → SE1 {2:+0.00;-0.00} · SE2 {3:+0.00;-0.00} · SE3 {4:+0.00;-0.00} · SE4 {5:+0.00;-0.00}\n", EnergyLayerData.DispatchBlocks[b], proxyMove, raised.ZonePrice[2][0][b] - untouched.ZonePrice[2][0][b], raised.ZonePrice[2][1][b] - untouched.ZonePrice[2][1][b], raised.ZonePrice[2][2][b] - untouched.ZonePrice[2][2][b], raised.ZonePrice[2][3][b] - untouched.ZonePrice[2][3][b]));
                }
                // (3) the reservoir
            ...
            sb.Append(F("\n    4. THE SHIFT: Italy's carbon tax raised {0:F0} EUR/t at the seed dispatch - the shiftable hydro restores the seed's peak–base spread\n", itRaise));
            {
                ...
                if (!(raisedIt.HydroShiftedGwh > 0)) { ok = false; Debug.LogError("RESERVOIR: Italy's carbon raise moved no hydro - the spread should have left the seed's and the operator answered."); }
            ...
            sb.Append("\n    5. B6: at a doubled price level, the continent doubled first, every Swedish zone's price doubles\n");
            {
                ...
                EnergyMarket.Result at1 = EnergyMarket.Clear(w.GetCountry(CountryId.Sweden), EnvironmentFamily.CarbonTaxRate(w.GetCountry(CountryId.Sweden)));
                foreach (Country c in w.Countries) { c.State.PriceLevel = 2f; }
                EnergyMarket.BeginTurn(w);
                EnergyMarket.Result at2 = EnergyMarket.Clear(w.GetCountry(CountryId.Sweden), EnvironmentFamily.CarbonTaxRate(w.GetCountry(CountryId.Sweden)));
        ...
        private static Outcome RunWorld(int years, CountryId? raiser, float raiseNational, double inflowScale)
        {
            ...
            try
            {
                ...
                sim.SetWorld(world);
                sim.PlayerCountryId = raiser ?? CountryId.Sweden;
                Country actor = raiser.HasValue ? world.GetCountry(raiser.Value) : null;
                float seedRate = actor != null ? EnvironmentFamily.CarbonTaxRate(actor) : 0f;
                ...
                for (int year = 1; year <= years; year++)
                {
                    ...
                    EnergyMarket.Result r = EnergyMarket.Clear(se, EnvironmentFamily.CarbonTaxRate(se));   // at the turn's water value and deficit share
            ...
            finally { UnityEngine.Object.DestroyImmediate(go); EnergyMarket.ProbeInflowScale = 1.0; }
        }
