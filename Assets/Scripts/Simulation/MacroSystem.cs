...
namespace PoliSim.Simulation
{
    ...
    public static class MacroSystem
    {
        ...
        public const float ParticipationElasticityToAfterTaxWage = 0.116f;   // §396: in the target, LANDED behind the jobs lag's two seats (§391, §394)
        ...
        public static void ApplyLaborForceParticipationRate(Country country, float reversionSpeed = LaborForceParticipationReversionSpeed)
        {
            ...
            float target = anchor
                ...
                + LaborTaxParticipationTerm(country);   // FT-5 LANDED as ruled (2026-09-08, §396): participation's response to the labour tax against its seed, SELMA's elasticity
            // FT-5's history in one place: built §372 (the multiplier read 1.33 at four years with unemployment FALLING - a participation rise was output the same
            // year through potential), REVERTED §383 and sheeted behind the jobs lag; measured behind its first seat §392 (unemployment RISING, +0.30 at two years)
            // and behind both §395 (0.79 at four years, +0.34); LANDED §396. The residual against SELMA's 0.17 is the consumption channel at the held MPC (FT-1), attributed there.
        ...
        /// the retired PopulationGrowthReversionSpeed documented (F2 step 4 deleted it with the scalar demography); life expectancy moves over decades, not quarters.</summary>
        /// <remarks>CONVENTION - a reversion speed, the rate at which a gap closes rather than a claim about the world.</remarks>
        ...
        // --- Demographics: RETIRED at F2 step 4 (2026-09-02). The eight demographic scalars are READINGS of the
        // cohort substrate now - see CohortDemographics. The rules that stepped them (a secular birth-rate decline,
        // three aging drifts, a growth-rate reversion, a population clamp, twelve constants) were deleted, not
        // reconciled: running both would have double-stepped the population (cohort spec-let §4.1).
