...
namespace PoliSim.Simulation
{
    ...
    public static class EnergyFleet
    {
        ...
        public static string CannotPlaceWhy(Country country, int technology, double mw, int year)
        {
            if (country == null) { return "NO COUNTRY"; }
            return CannotOrderWhy(country.Id, technology) ?? EnergyConnectionQueue.RefusalFor(country, technology, mw, year);
        }
        ...
        public static Order Place(Country country, int technology, double mw, int year, int turn)
        {
            ...
            if (mw > 0 && EnergyConnectionQueue.RefusalFor(country, technology, mw, year) != null) { return null; }   // P6-F2e (§551): a build past the queue's published capacity is refused whole - CannotPlaceWhy has the sentence
            if (mw < 0)
            {
