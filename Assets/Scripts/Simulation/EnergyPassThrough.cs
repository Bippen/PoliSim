...
namespace PoliSim.Simulation
{
    ...
    public static class EnergyPassThrough
    {
        ...
        public static float PlannedForPreview(Country preview) => PlannedForPreview(preview, out _);

        /// <summary>EN-7b: the same, handing back the clone's book (null where the layer does not run) - so the preview reads the electricity tax's budget flow from the one clearing.</summary>
        public static float PlannedForPreview(Country preview, out EnergyLedger.Book book)
        {
            book = null;
            ...
            book = EnergyLedger.Compute(preview, EnergyMarket.Clear(preview), priceIndex, EnergyLedger.CreditFor(preview));
