...
try {
  ...
  $money = 'Fiscal|Budget|Tax|Spending|Debt|Ledger|SovereignWealthFund|Welfare|Pension|EnergyMarket|EnergyPassThrough|AiFinanceMinistry|TradeCosts|Transfer|CarbonRate|ProgramBill|PortfolioEffectiveness|EconomyState|EnergyFleet|EnergyConnectionQueue|AiEnergyMinistry|SimulationManager|MacroSystem'   # s546: the fleet decides what is dispatched and so what is billed - s539's defect lived in EnergyFleet.cs. s549 (ruled 2026-09-21): SimulationManager - the fiscal path cannot sit outside a money-path audit because of its name. s551: EnergyConnectionQueue - every refusal in EnergyFleet.Place is decided there (its own review's finding). s575 (ruled 2026-09-21, s571 item 3): MacroSystem - it moves the price level every figure in the book is deflated by, so it books by another name, which is the class SimulationManager was named in for
