...
try {
  ...
  $money = 'Fiscal|Budget|Tax|Spending|Debt|Ledger|SovereignWealthFund|Welfare|Pension|EnergyMarket|EnergyPassThrough|AiFinanceMinistry|TradeCosts|Transfer|CarbonRate|ProgramBill|PortfolioEffectiveness|EconomyState|EnergyFleet|EnergyConnectionQueue|AiEnergyMinistry|SimulationManager|MacroSystem|AuthoredImpactScale'   # s546: the fleet decides what is dispatched and so what is billed - s539's defect lived in EnergyFleet.cs. s549 (ruled 2026-09-21): SimulationManager - the fiscal path cannot sit outside a money-path audit because of its name. s551: EnergyConnectionQueue - every refusal in EnergyFleet.Place is decided there (its own review's finding). s575 (ruled 2026-09-21, s571 item 3): MacroSystem - it moves the price level every figure in the book is deflated by, so it books by another name, which is the class SimulationManager was named in for
  ...
  if ($touched -contains 'SIMULATION') {
    if ($moneyHits.Count) {
      ...
      foreach ($m in $moneyHits) {
        ...
        else { "    ledger   : $m at $($state.Substring(0, 8)) - NO ROW FOR THIS STATE. If the change is to COMMENTS ONLY, ReviewLedgerCheck accepts it on the reviewed state before it (s588: the compiler's parser, Roslyn in Tools/textcheck, finds the code identical, walked back through git) and prints 'comment-only'; otherwise it fails the cheap bar until the review has run, its report is under Reviews/ and Tools/review_row.ps1 has added the row" }
      }
    }
