...
# the national currency per unit of the market's currency (EUR for the five, USD for the USA): 1 for the euro members and the USA; ECB 2023 reference rates for the krona and the zloty
my %nationalPerMarket = (DE => 1, FR => 1, IT => 1, US => 1); { my @l = split /\n/, slurp("$src/ecb_sek_pln_eur_annual.csv"); for (@l) { my @c = split /,/; next unless ($c[6] // '') eq '2023'; $nationalPerMarket{SE} = $c[7] if $c[2] eq 'SEK'; $nationalPerMarket{PL} = $c[7] if $c[2] eq 'PLN'; } } die "ECB SEK/PLN" unless $nationalPerMarket{SE} > 1 && $nationalPerMarket{PL} > 1;
...
my %avail = (coal => 0.85, gas => 0.85, oil => 0.85);   # [AUTHORED-DRAFT] technical availability of a thermal fleet net of outages; the ERAA de-rating row is BILLED
{
    ...
    printf $o "# national_per_market: the country's currency per unit of the market's - 1 for the euro members and the USA, the ECB 2023 reference rates for Sweden (%.4f SEK/EUR) and Poland (%.4f PLN/EUR); the carbon tax's points are national currency per tonne and enter the merit order through it.\n", $nationalPerMarket{SE}, $nationalPerMarket{PL};
    printf $o "# usd_per_market: the BOOK's dollars per unit of the market's currency - the game keeps every country's book in US dollars - the ECB 2023 reference rate %.4f USD/EUR for the five, 1 for the USA; the fiscal layer's stack and ledgers are in the book's dollars through it.\n", $fx;
    print $o "country;category;fuel_per_mwh_th;efficiency;ef_t_per_mwh;vom_per_mwh;availability;main_share;lignite_share;ets_per_t;national_per_market;usd_per_market\n";
    for my $cc (@eu, 'US') { for my $cat (qw(coal gas oil)) {
        ...
        print $o join(';', $cc, $cat, f2($fuel), f4($eff), f4($ef), f2($vom), f2($avail{$cat}), f4($main > 1 ? 1 : $main), f4($lig), f2($ets{$cc}), f4($nationalPerMarket{$cc}), f4($cc eq 'US' ? 1 : $fx)), "\n"; } }
    close $o;
}
