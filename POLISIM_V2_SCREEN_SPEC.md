...
**Built (Phase B, 2026-08-28):** in order — the §E5 close (the hatch pair's third cut measured, the bar question filed), D6, D4, 1n-r2, 1m-r2, 2a — one unit one commit; the full `v31b_*` matrix at four sizes; Annex F and the dead-space share re-measured and filed back. The build's own reversible calls are R-F1… (`COMPLETED.md` §45).
