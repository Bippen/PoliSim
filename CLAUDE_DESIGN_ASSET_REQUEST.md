...
<!-- GENERATED by D18Inventory.Emit (Tools/d18_inventory.sh). DO NOT EDIT BY HAND. source-digest: 0ec343517edd7f836b74c6e7fd489381d647f34966f9da5afa2698c6b03f2d7c -->
...
#### 1 Â· Party identity marks â `PartyMarkCoverageCheck`

```
  no mark   PartySystems/Christlich Demokratische Union - MarkName unset; the row draws without one
  no mark   PartySystems/Alternative fur Deutschland - MarkName unset; the row draws without one
  no mark   PartySystems/Sozialdemokratische Partei - MarkName unset; the row draws without one
  no mark   PartySystems/Bundnis 90/Die Grunen - MarkName unset; the row draws without one
  no mark   PartySystems/Die Linke - MarkName unset; the row draws without one
  no mark   PartySystems/Christlich-Soziale Union - MarkName unset; the row draws without one
  no mark   PartySystems/Sudschleswigscher Wahlerverband - MarkName unset; the row draws without one
  no mark   PartySystems/Bundnis Sahra Wagenknecht - MarkName unset; the row draws without one
  no mark   PartySystems/Freie Demokratische Partei - MarkName unset; the row draws without one
  no mark   PartySystems/Union de la gauche (NFP joint candidacies) - MarkName unset; the row draws without one
  no mark   PartySystems/Ensemble (majorite presidentielle) - MarkName unset; the row draws without one
  no mark   PartySystems/Rassemblement National - MarkName unset; the row draws without one
  no mark   PartySystems/Les Republicains - MarkName unset; the row draws without one
  no mark   PartySystems/Divers droite - MarkName unset; the row draws without one
  no mark   PartySystems/Union de l'extreme droite (RN-Ciotti) - MarkName unset; the row draws without one
  no mark   PartySystems/Divers gauche - MarkName unset; the row draws without one
  no mark   PartySystems/Regionaliste - MarkName unset; the row draws without one
  no mark   PartySystems/Horizons - MarkName unset; the row draws without one
  no mark   PartySystems/Divers centre - MarkName unset; the row draws without one
  no mark   PartySystems/Union des democrates et independants - MarkName unset; the row draws without one
  no mark   PartySystems/Parti socialiste (outside the UG banner) - MarkName unset; the row draws without one
  no mark   PartySystems/Ecologistes - MarkName unset; the row draws without one
  no mark   PartySystems/Divers - MarkName unset; the row draws without one
  no mark   PartySystems/Extreme droite - MarkName unset; the row draws without one
  no mark   PartySystems/Fratelli d'Italia - MarkName unset; the row draws without one
  no mark   PartySystems/Partito Democratico - MarkName unset; the row draws without one
  no mark   PartySystems/Lega per Salvini Premier - MarkName unset; the row draws without one
  no mark   PartySystems/Movimento 5 Stelle - MarkName unset; the row draws without one
  no mark   PartySystems/Forza Italia - MarkName unset; the row draws without one
  no mark   PartySystems/Azione - Italia Viva - MarkName unset; the row draws without one
  no mark   PartySystems/Alleanza Verdi e Sinistra - MarkName unset; the row draws without one
  no mark   PartySystems/Noi Moderati - MarkName unset; the row draws without one
  no mark   PartySystems/Sudtiroler Volkspartei - PATT - MarkName unset; the row draws without one
  no mark   PartySystems/Piu Europa - MarkName unset; the row draws without one
  no mark   PartySystems/Impegno Civico - MarkName unset; the row draws without one
  no mark   PartySystems/Sud chiama Nord - MarkName unset; the row draws without one
  no mark   PartySystems/MAIE (circoscrizione Estero) - MarkName unset; the row draws without one
  no mark   PartySystems/Union Valdotaine (VdA college) - MarkName unset; the row draws without one
  no mark   PartySystems/Prawo i Sprawiedliwosc - MarkName unset; the row draws without one
  no mark   PartySystems/Koalicja Obywatelska - MarkName unset; the row draws without one
  no mark   PartySystems/Trzecia Droga - MarkName unset; the row draws without one
  no mark   PartySystems/Nowa Lewica - MarkName unset; the row draws without one
  no mark   PartySystems/Konfederacja - MarkName unset; the row draws without one
...
=== Party marks: 53 seeded part(ies), 10 with a resolving mark, 43 without one, 0 delivered-but-unconsumed (ceiling 0), 0 error(s) ===
```

**By chamber** (seeded parties, the marks that resolve, and what is left to cut; the rule
`mark_party_<iso2>_<abbrev>` re-derived against all 10 delivered marks on this run):
...
|---|---|---|--:|---|
| Germany | CDU | Christlich Demokratische Union | 164 | `mark_party_de_cdu` |
| Germany | AfD | Alternative fur Deutschland | 152 | `mark_party_de_afd` |
| Germany | SPD | Sozialdemokratische Partei | 120 | `mark_party_de_spd` |
| Germany | Grune | Bundnis 90/Die Grunen | 85 | `mark_party_de_grune` |
| Germany | Linke | Die Linke | 64 | `mark_party_de_linke` |
| Germany | CSU | Christlich-Soziale Union | 44 | `mark_party_de_csu` |
| Germany | SSW | Sudschleswigscher Wahlerverband | 1 | `mark_party_de_ssw` |
| Germany | BSW | Bundnis Sahra Wagenknecht | 0 | `mark_party_de_bsw` |
| Germany | FDP | Freie Demokratische Partei | 0 | `mark_party_de_fdp` |
| France | UG | Union de la gauche (NFP joint candidacies) | 178 | `mark_party_fr_ug` |
| France | ENS | Ensemble (majorite presidentielle) | 150 | `mark_party_fr_ens` |
| France | RN | Rassemblement National | 125 | `mark_party_fr_rn` |
| France | LR | Les Republicains | 39 | `mark_party_fr_lr` |
| France | DVD | Divers droite | 27 | `mark_party_fr_dvd` |
| France | UXD | Union de l'extreme droite (RN-Ciotti) | 17 | `mark_party_fr_uxd` |
| France | DVG | Divers gauche | 12 | `mark_party_fr_dvg` |
| France | REG | Regionaliste | 9 | `mark_party_fr_reg` |
| France | HOR | Horizons | 6 | `mark_party_fr_hor` |
| France | DVC | Divers centre | 6 | `mark_party_fr_dvc` |
| France | UDI | Union des democrates et independants | 3 | `mark_party_fr_udi` |
| France | SOC | Parti socialiste (outside the UG banner) | 2 | `mark_party_fr_soc` |
| France | ECO | Ecologistes | 1 | `mark_party_fr_eco` |
| France | DIV | Divers | 1 | `mark_party_fr_div` |
| France | EXD | Extreme droite | 1 | `mark_party_fr_exd` |
| Italy | FdI | Fratelli d'Italia | 119 | `mark_party_it_fdi` |
| Italy | PD | Partito Democratico | 69 | `mark_party_it_pd` |
| Italy | Lega | Lega per Salvini Premier | 66 | `mark_party_it_lega` |
| Italy | M5S | Movimento 5 Stelle | 52 | `mark_party_it_m5s` |
| Italy | FI | Forza Italia | 45 | `mark_party_it_fi` |
| Italy | AzIV | Azione - Italia Viva | 21 | `mark_party_it_aziv` |
| Italy | AVS | Alleanza Verdi e Sinistra | 12 | `mark_party_it_avs` |
| Italy | NM | Noi Moderati | 7 | `mark_party_it_nm` |
| Italy | SVP | Sudtiroler Volkspartei - PATT | 3 | `mark_party_it_svp` |
| Italy | PlusE | Piu Europa | 2 | `mark_party_it_pluse` |
| Italy | IC | Impegno Civico | 1 | `mark_party_it_ic` |
| Italy | ScN | Sud chiama Nord | 1 | `mark_party_it_scn` |
| Italy | MAIE | MAIE (circoscrizione Estero) | 1 | `mark_party_it_maie` |
| Italy | UV | Union Valdotaine (VdA college) | 1 | `mark_party_it_uv` |
| Poland | PiS | Prawo i Sprawiedliwosc | 194 | `mark_party_pl_pis` |
| Poland | KO | Koalicja Obywatelska | 157 | `mark_party_pl_ko` |
| Poland | TD | Trzecia Droga | 65 | `mark_party_pl_td` |
| Poland | NL | Nowa Lewica | 26 | `mark_party_pl_nl` |
| Poland | Konf | Konfederacja | 18 | `mark_party_pl_konf` |
...
*154 sprite(s) under `Assets/Resources/Art/UI/`. A file counts as REACHED when an
...
**The runs behind this block**: PartyMark 0 Â· Portrait 0 Â· StatIcon 0 Â· AreaIcon 0 Â· AudioCue 0 â 0 is a clean check; a non-zero code means that family's own bar is red and this
