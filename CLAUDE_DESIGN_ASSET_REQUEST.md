...
## ⭐ STATUS 2026-09-17 (night) — **D21 IS THE LIVE ASK: the Energy tab's composition with the player's decisions as its spine, the area's icon and ink that the rail cell now needs, the pie's size and the bar it falls back to, three forms the People page's measurement says it needs, and the selector card at the smallest window - reconciled against D20, nothing already answered re-asked** (§ ⭐ D21 below). Playtest 6 (2026-09-17, `COMPLETED.md` §526–§536) put Energy on the rail by Elias's ruling (DS-4b overruled, "the player's reading wins"); every other row here is a measurement this pass made and a form only you can draw. Sending stays Elias's (E-41).

## ⭐ D21 — THE ASK (2026-09-17)

**Read against D20 first.** D20's three boards are answered and built (15a the energy page's plates against 14a; 15b the schedule row's grammar; 15c the statutory marks) and none of that is re-asked: the plates, the ledger, the instruments and the marks stand as you drew them. What is asked is what the pass measured and could not draw.

| # | the row | what is asked | the measurement behind it |
|---|---|---|---|
| **1** | **`icon_area_energy` and the area's ink** | The rail has a ninth cell, ENERGY, in `UiPalette.SystemArea.Energy`'s ink. There is no icon on disk - `AreaIconCoverageCheck` reports it MISSING on every bar, by ruling, until yours lands - and no ink of its own: the cell and the page borrow the Sectors indigo as a stated stand-in. Asked: the icon in the `icon_area_*` idiom (white on alpha, RGBA32, the 24-grid the other ten sit on) and the area's ink pair (the paper ink at 4.5:1 on the card, and the lifted desk weight). | The rail's missing-icon fallback draws the caption's initial in the area's ink (a tinted E), which navigates but does not read as the others do. Filmed at 1280 and 2560 (`COMPLETED.md` §536). |
| **2** | **The Energy tab's composition** | The page moved whole to its own tab and it is the layer's readouts - the plates D20 answered. What it lacks is anything a player DOES. The spec-let's stages name the decisions and none is invented here: build and retire (capacity by technology with CAPEX, lead time and a connection queue), the four policy instruments as dials with their range captions, the law category EN-7b built reachable from the tab, and the ministry on the finance ministry's pattern with its mandate weights. Asked: the tab's composition with those decisions as its spine and the readouts as their consequence - where the decision surfaces sit relative to the plates, and what a decision looks like on this page. | Elias's sheet, verbatim in §526: *"clear, simple, and the choices are the point."* The plates as they stand are filmed at both widths on the tab (§536). |
| **3** | **The pie below its size, and the bar it becomes** | The distribution pie is a 120 px disc at every window, and 8a's inside-label rule (a share printed where the wedge clears 45°) is over its own chord at every filmed width: the label needs 32.1 px at 1280 against a 28.5 px chord, and 53.6 px at 2560. Below that threshold the renderer now draws the distribution as the stacked bar the grammar already has (the plate's distribution band's form). Asked: whether the disc should scale with the window so the pie holds at size, or the bar is the form above 1280 - and if the bar, its own board (the legend's place, the OTHER's hatch on a bar). | The label table's own widths against the renderer's chord (§535); both forms filmed. |
| **4** | **The family's key figure in its header** | The People page's family header carries the name and, behind the tab, the publisher and vintage; it has no figure cell. The figure is what a player reads first (the largest type and the darkest ink, in one column), and today the first one a family shows is a row's. Asked: a header form with the family's headline figure, so a family reads before its rows do. | §531: rows per viewport, ink per column, what reads first. |
| **5** | **A per-row disclosure** | A row's detail (the reach chips, the unit and source lines, the rank, the honesty column, the seed foot) opens with the desk-global `†` and sits closed without it - every row at once, and the state persists across sessions by §9.1. Asked: whether a row's own detail should open in place, on that row, beside the global tab - and its form if so. | §532; the tab's persistence is stated there as Elias's ruling to keep or revisit. |
| **6** | **The extra row's form** | The health family's *Supporting readouts* row carries fifteen draws at rest (five sub-figures, each with a caption above and a unit line below) against three to four on any other row. Asked: what that row should carry at rest. | §531's ink table. |
| **7** | **The selector card at the smallest window** | The card's labels were asked for at 5.9 device pixels at 1280×720 and were mush; a device-pixel floor now holds every Canvas line at the desk's caption floor, and at 1280 that puts the card's figure label and its hue line on the same size - two levels of the card's hierarchy flattened. Asked: the card's composition at the smallest geometry, now that nothing on it may fall below the floor. | §527's scale table and the magnified crops, before and after. |
