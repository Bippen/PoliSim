...
## ⭐ STATUS 2026-09-21 (night) — **D21 ROW 1 IS INSTALLED (your 16a: the bolt and the olive ink - the bar reads 44 of 44 for the first time since the tab was born), YOUR 16b–16e ARE READ AND NOT YET BUILT, AND THE LIVE ASK IS A SITTING: THE WHOLE GAME, FILMED, FOR A READING OF THE DESK AS ONE - NOTHING ASKED AS A REDRAW** (`COMPLETED.md` §556, §560; the archive `SITTING_2026-09-21.zip`, E-42)

**16a, installed as delivered.** `send/icon_area_energy.png` verified by content (your MANIFEST's digest; 256 × 256 RGBA, one white tone on alpha, the 214 box on the 24-grid with its 21 px air, seated on the lower edge) and imported beside its ten siblings. The ink `#566217` and the lift `#8A9648` are in the palette; the electricity-tax law's area flipped to Energy in the same cut. Your two bars recompute here from the hexes and reproduce: 5.03:1 on the active tongue, 4.96:1 on the rail's desk ground; Political the nearest area ink. **Four things back from installing it, none a defect of the file:** (1) the README's split of the inked pixels is thirty off the file's on the same total, and the file carries a content-credentials chunk its ten siblings do not - both harmless; (2) the board's list of area hues is not this repo's on-paper inks' in OKLCH by ten to twenty degrees in several places - Political is the nearest neighbour either way; (3) **16a delivers no inactive tab-swatch tint** and says it derives *as the other ten do* - the theme's table holds six SNAPPED tints from pass 3 under a stated rule (the ink at oklch chroma ×0.78, lightness ×0.97), so Energy's was computed by that rule (`#535D28`), and the rule does not reproduce your six to the unit; (4) **your first rail flag measures a rail we did not build** - 1n-r3 as built draws every cell as a PAPER tongue, so the inactive glyph is that tint on paper, not the lift at 0.72 on the desk ground. (3) and (4) ride in the sitting with your second flag.

**16b, 16c, 16d, 16e - read, recorded, not built.** They stand as your answers to D21 rows 2 to 5; nothing in them is disputed and nothing of them is on a frame yet.

**The sitting.** One archive: every surface a player can reach, at 1280 and at 2560, from one real game - the play protocol's save loaded through the game's own Load and played forward by its own day path - each frame by digest with one line naming the screen and its state, and each proving the screen it claims. **What is asked is a reading of the desk as one: where it reads well, where it reads badly, what has drifted between boards built weeks apart. Nothing is asked as a redraw and no board is owed.** The head names what is known on the frames and ours, so you need not report it, and carries the composition ask's three rows so this is one paste: the Energy tab composed with its decisions as its spine; C2's three density rows with your 16b–16d standing as their answers; the rail's two token flags.

## ⭐ STATUS 2026-09-21 (Part 2 re-cut against the tab as it now stands; Part 1 stands as asked on 2026-09-18) — **D21 IS THE LIVE ASK, IN TWO PARTS: asked now, the Energy area's icon and ink (the rail cell that stands with a tinted E), three forms the People page's measurement says it needs, and the pie's size and the bar it falls back to; ARRIVING, the Energy tab's composition - the tab exists, is reachable and holds your D20 plates; its first decision (build and retire) is on it with its refusals as sentences and the country's own energy mandate under the queue, the four instruments are dials, a chip opens the electricity tax's laws, and the AI ministry is built and held - and the composition ask follows against the built page the D19 way, not against the spec-let's text. Reconciled against D20; nothing answered is re-asked.** (§ ⭐ D21 below.) The rows are Playtest 6's (`COMPLETED.md` §526–§539); Part 2's re-cut is §547, on §542–§544. Sending stays Elias's (E-41): one archive, `AssetPackArchive/D21_ask.zip`, cut with `MANIFEST.sha256` inside and verified both ways.

## ⭐ D21 — THE ASK (2026-09-18), IN TWO PARTS; PART 2 RE-CUT 2026-09-21

**Read against D20 first.** D20's three boards are answered and built (15a the energy page's plates against 14a; 15b the schedule row's grammar; 15c the statutory marks) and none of that is re-asked: the plates, the ledger, the instruments and the marks stand as you drew them. Part 1 is what the pass measured and could not draw; Part 2 says what is coming and gives you the page as it stands, so the ask that follows is drawn against real parts.

### Part 1 — asked now

| # | the row | what is asked | the measurement behind it |
|---|---|---|---|
| **1** | **`icon_area_energy` and the area's ink** | The rail has a ninth cell, ENERGY, and no art for it: `AreaIconCoverageCheck` reports `icon_area_energy` MISSING on every bar - by ruling, until yours lands (every cheap bar reads 42 of 43 meanwhile) - and the area has no ink of its own, so the cell and the page borrow the Sectors indigo as a stated stand-in. Asked: the icon in the `icon_area_*` idiom (white on alpha, RGBA32, the 24-grid the other ten sit on) and the area's ink pair (the paper ink at 4.5:1 on the card, and the lifted desk weight). | The rail's missing-icon fallback draws the caption's initial in the stand-in ink - a bare E fitted to the slot - which navigates but does not read as the others do. The rail at nine cells is filmed at 1280 and 2560, active and inactive (`COMPLETED.md` §536). |
| **2** | **The family's key figure in its header** | The People page's family header carries the name and, behind the tab, the publisher and vintage; it has no figure cell. Asked: a header form with the family's headline figure, so a family reads before its rows do. | §531, off the label tables and the pixels: a data row at rest is about 5 % ink; **the figure is what a player reads first** - the largest type in the row (17 units, bold) and the darkest ink in every data row (38, the page's black), in one column with one left edge - and today the first one a family shows is a row's, not the family's. Rows per viewport at 1280×699: 17 row names under 4 family headers, the pitch 36 px; at 2560×1419, 18 under 4, the pitch 66. |
| **3** | **A per-row disclosure** | A row's detail - the reach chips, the unit and source lines, the rank numeral, the honesty column, the seed foot - opens with the desk-global `†` and sits closed without it: every row at once, and the state persists across sessions (§9.1). Asked: whether a row's own detail should open in place, on that row, beside the global tab - and its form if so. | §532: at rest a row is one glance - 3 to 4 text draws and three graphics (the band with its tick and peer ticks, the pips, the sparkline); with the tab open every row carries its apparatus on every visit until the tab is closed. |
| **4** | **The extra row's form** | The health family's *Supporting readouts* row carries fifteen draws at rest - five sub-figures, each with a caption above and a unit line below - against three to four on any other row. Asked: what that row should carry at rest. | §531's ink table: the row's figure column reads 8.1 % ink against 3.1–5.6 % on the family's data rows, its band column 7.7 % against 0.0–4.5 %. |
| **5** | **The pie below its size, and the bar it becomes** | The distribution pie is a 120 px disc at every window, and 8a's inside-label rule (a share printed where the wedge clears 45°) is over its own chord at every filmed width, so the renderer draws the distribution as the plate grammar's stacked bar there - at every width this project films. Asked: whether the disc should scale with the window so the pie holds at size, or the bar is the form - and if the bar, its board (the legend's place, the OTHER's hatch on a bar). | §535: the inside share label needs 32.1 px at 1280 against a 28.5 px chord, and 53.6 px at 2560 against the same 28.5; both forms filmed at both widths. |

**Held back, not asked this round:** the selector card at the smallest window (§527's floor holds every Canvas line at the desk's caption floor and flattens two levels of the card's hierarchy at 1280) - a finding kept, to be asked against a played card if Elias's play says so.

### Part 2 — ARRIVING: the Energy tab's composition *(re-cut 2026-09-21 against the tab as it now stands; the text of 2026-09-18 described a page with one decision and three promises, and all three have landed)*

- **The tab exists and is reachable.** Energy is the rail's ninth cell (DS-4b overruled, §536) and the page moved whole to it: the layer's readouts - the plates you answered in D20/15a - under its own header, at both widths.
- **Its first decision is on it, and it now works in play.** Build and retire (§539, re-founded in §544): a line per technology with the fleet it has and its lead time (EIA AEO2023 Table 1: coal 4 y, gas 3, nuclear 6, wind 3, solar 2), a step down and a step up, the connection queue with each order's year placed and the year it connects or leaves. **The queue refuses what would not move, and each refusal is a sentence on the technology's own line with its steppers drawn disabled** - this is new since the first cut, and in Sweden four of the plate's seven lines are refusals: hydro (*THE WATER, NOT THE TURBINE, IS THE LIMIT*) and other (*NO SINGLE TECHNOLOGY, NO LEAD-TIME ROW*) everywhere; nuclear where the law has closed it (Germany: *CLOSED BY LAW · A BUILT MW WOULD NOT RUN*); coal and gas where the record holds no such fleet (France's and Italy's coal: *THE RECORD'S FUEL GROUPS DO NOT HOLD THESE PLANTS*) or where the zones clear without one (Sweden: *THE ZONES CLEAR WITHOUT A FOSSIL FLEET · NOTHING WOULD MOVE*). The cost row still reads **BILLED** - IRENA's 2024 costs are the CAPEX source and they are owed, so nothing is priced and the page says so.
- **The mandate stands under the queue** (§544): the country's own energy statute read for the player - a citation line (*THE MANDATE · BET. 2022/23:FIU21 · RIKSDAGEN*; Germany's reads *EEG 2023 § 4 · WINDSEEG § 1 · KVBG § 4*), the statute's figures with what the fleet and the queue reach against them (*WIND 145 GW · SOLAR 215 GW IN 2030 · COAL OUT BY 2038 · WITH THE QUEUE: …*), and a closing line saying that an AI state's ministry answers its own statute with orders like these and that this is HELD. Poland's reads BILLED (its plan was reached through secondary reports only) and the USA's says there is no federal statutory target to answer. **The AI ministry is built and held: no order of its is on any page yet.**
- **The four instruments are dials now** (§542) - *Retail intervention*, *Market liberalisation*, *Investment planning*, *State ownership*: the Energy sector's own four dials under the names the spec-let's S9 gave them, in the slider-row family you drew for D13 (end names under the track, the figure at the right, the range caption in the band while a knob is held), sharing one draft with the Sectors page and one bill, with the bill's status and its one button beneath them. They sit at the page's foot, under the readout rows that have carried the same four names since EN-6; two of the four (*Investment planning*, *State ownership*) reach nothing in the energy layer and their readout rows say so as ABSENT, in place. Where the retail stack carries no policy levy (the USA) the first dial keeps the sector's own name, *Subsidy*.
- **The law category is reachable from the tab** (§543): one faced chip between the readout rows and the dials - *THE ELECTRICITY TAX'S LAWS ▸* - with a line beside it saying what it opens and what it will find there (*OPENS LAWS ON THIS CATEGORY · 10 OFFERED · 0 ENACTED · 0 BEFORE PARLIAMENT*); drawn disabled with its reason where the country has no national statute (the USA). Filmed at rest and arrived (`filmpa3_1280_08_energy_laws_link`, `…_laws_link_arrived`).
- **What is still to come before the composition is asked:** a capacity on the connection queue (the ministry's first prerequisite, its own item - today an order of any size is accepted), the capital-cost row when IRENA's costs land, and the ministry's orders on an AI country's page once it is live.
- **The composition ask follows the D19 way** - against the built page with the decisions on it, real parts, not against the spec-let's text. Until then nothing in Part 2 is asked. **One fact about the page as it stands, for when you draw against it:** its order is the build order, not a composed one - the rule row and the stacks, the fleet and the zones, *Build and retire* (the order, the queue, the mandate, the BILLED cost), the water, the system cost and the incidence bridges, then the instruments' readout rows, the laws chip, the four dials and the bill. The decisions sit mid-page between readouts and the dials at its foot. The page is in the archive with its ABSENT rows honest: `Load growth · ABSENT · THE LOAD DOES NOT GROW WITH GDP OR ELECTRIFICATION · STATED, NOT MODELLED`, and the *Investment and retirement* row that read ABSENT since EN-6 is retired, because the decision exists.

**What is not asked.** The energy plates (D20/15a), the schedule row (15b), the statutory marks (15c), the plate grammar (9c/10a), the eight party inks (D17), the mark inventory (D18) - all answered and built; the pass touched none of their forms.

## ⭐ STATUS 2026-09-16 (evening) — **YOUR r3c NOTE IS READ: THE ARCHIVE LANDED WHOLE (27 OF 27, THE DIGESTS ARE THE MANIFEST'S), THE ELEVEN CORRECTIONS ARE ON THE BOARD AND EACH ANSWERS THE FLAG IT NAMES, AND ONE PRECONDITION MOVED SINCE YOU WROTE - THE PENSIONS DRIVER IS BUILT (`COMPLETED.md` §520), SO THE OVERRIDE STATE NOW WAITS ON THE DIAL, WHICH IS NOT. NOTHING IS ASKED, EITHER WAY.** Read against your note's own claims (`COMPLETED.md` §521): the request document you name is the one the archive carried (`09a311c7…`, 220 909 B) and the CSV is the same six statutes (`ef7bb488…`); the knob geometry you quote (Sweden's mark at 647–655 under a face ending at 651, Germany's label centred at 615 over its tick at 616) is §515's own measurement; *"the FIXED and USA-COMPLETE frames byte-identical across years"* holds as stated - Poland's three frames are one file, the USA's two COMPLETE frames are one file and its 2026 frame differs because 2026 is the schedule's last open year (CLOSED, `66 y 10 m · 2026` ahead of the knob), Sweden's two HELD frames are one file and its CARRIED frame another. Flags 1–9 stand corrected where they were made and flags 10–11 (the counts) in the archive cards; the board's rule is untouched, as your note says. **What moved:** PN-1's driver landed the same day (§520) - the pension line now follows the cohort at the age the statute has in force, read through the payment row's own accessor, so *"PENSIONS DRIVER DEFERRED BY RULING"* is no longer the reason the override state is unbuilt. The reason is the DIAL (DS-3's *a passed bill's figure wins*), which stays open; the ghost tick, the hairline from the ghost and the bill's figure are drawn the day a bill can set an age, and that day is not asked of you. The status that stood here is kept beneath.

## ⭐ STATUS 2026-09-16 — **YOUR 15c-r3 IS READ AND THE FOUR THINGS IT CAUGHT ARE BUILT (`COMPLETED.md` §515); ELEVEN THINGS GO BACK, AND NOTHING IS ASKED.** The board was read the D11 way, claim by claim: one hundred claims, seventy-six clean. **Built here:** the row's marks now key to the knob sprite's own FACE, measured off its alpha (30×46, the opaque face x 3–26, y 2–41), not to the thumb's rect - so the held mark PROTRUDES BELOW THE KNOB as your device says (on §509's film it stopped inside the face: the face ran y 623–651 and the mark's ink 645–650), a step the face covers brackets the knob instead of shading it (Germany 2034 and Italy 2029: two months from the knob is six pixels at 1280, and at half ink that read as the sprite's own shading), and a cut label stands over the tick it names (`67 · 2031` printed eleven pixels left of its own tick). **Eleven things back, each checked against a frame or the statute layer:** Sweden is CARRIED from 2033, not 2034; Italy's RISING window is 2026 and 2027, with 2028 a one-year held window and CARRIED from 2029 as your own cell says; no frame prints the HELD sentence your specimen recites at 1280 (the row prints `NEXT: 2033 NO FIGURE YET`, and your own rule about the citation dropping first is what it does); France's two clauses print in the other order; the `60` and `70` at the track's ends are not drawn, because the band carries the sentence; **four marks are this row's own that D13 and the gradation do not have** (the dotted hairline, the half ink, a state tick at twice the gradation's width, and now the bracket) - stated, not smuggled; FIXED's empty track is empty ahead of the knob only; CARRIED's device draws for Italy and not for Sweden, whose path has one point; `66 · THE BILL` is in no CSV row; and the annex count names sixteen files for twelve frames (the archive carries eleven `clk_*` and `r2_france_2560`, with five superseded `sp2_*` beside them). Your COMPLETE re-cut (`66 y 10 m · 2029`), the USA's cell, Germany's cluster, France's three steps, Italy's two and Sweden's window are all what the row prints. **Nothing is asked.** The status that stood here is kept beneath.
...
## ⭐ STATUS 2026-09-13 (night) — **D20 IS ANSWERED (your boards 15a, 15b, 15c, one per batch) AND ALL THREE ARE BUILT (`COMPLETED.md` §486, §490, §491); NOTHING IS LIVE TO DESIGN.** The paste went up as `uploads/D20_paste/` with every digest, and the three boards came back the same day. **15a** (the energy page composed over the built page) was read the D11 way and taken whole - it asserts no instruction the record lacks, strikes 14a's inventions itself, answers the rail cell NO - and is BUILT and filmed in six countries at two widths (`COMPLETED.md` §486): the rule row as one device with the parts under their names, the wholesale as its head and the derivation as its foot, the stacks on one scale, the links in the gutters with BINDS as a mark and a word, the water value under the blocks, the two bridges on one scale with one leader, the absent rows in place. Four small deviations are stated in that record (the legend line in the plate foot, the REACHES chips in PROVENANCE as 9c has them, the illustrative Δ tick not built, the head and body as two rows of one plate). **15b** (the schedule row) rides on the schedule class it samples - F4-2, the tax chain's first family - and closes F4-4 with it; **15c** (the statutory mark) rides on the pension dial row - PN-1. Both read clean and are recorded as answered on their rows. The status that stood here is kept beneath.

## ⭐ STATUS 2026-09-12 (night) — **D20 IS THE LIVE ASK: the energy page reconciled against your board 14a over the page as built and filmed, the schedule row's grammar on six sourced tariff shapes, the statutory mark on a lever the law also moves** (§ ⭐ D20 below). The energy page is BUILT and filmed in six countries at two widths (`COMPLETED.md` §481) - four plates, instruments first, every figure the state's book or the market's own clearing, everything the layer lacks drawn as an ABSENT row with its reason. Your 14a (2026-09-11, drawn before the build from the spec-let) was read live the D11 way and is taken as your answer to the spec-let's board ask: its forms match the page to the point that batch 1 is a reconciliation, not a composition from a description - what matches is taken, what the page holds that 14a never saw is asked (the rule on the price in TWO forms - the merit order for five countries, the reservoir dispatch for Sweden's zones, the row saying which priced the block; and the absent rows as content, not gaps), what 14a holds that the game does not is said and not asked. Boards 12a–12f and the redirect note are read board by board: each rests on an instruction this repo's record does not hold and stops as a board - no instruction is recorded as Elias's - and their own grammar is read back as yours. Two figures corrected for the spine, one file held for re-issue. **Beneath D20, everything E-32 never pasted is carried whole.** The status that stood here is kept beneath.

## ⭐ STATUS 2026-09-10 (evening) — **D19 IS ANSWERED AND READ (`COMPLETED.md` §449); 13b IS BUILT (§450) AND 13a IS STOPPED ON THIS SIDE.** Design answered both items on boards 13a and 13b. 13b - the ledger as a bridge - checked figure for figure against the repo and is built, with the campaign map moved to 4a's arrangement on its note 2. 13a's composition checks out, but three of its citations are not in the record and one overturns a ruling this side holds (B8, the interrupt banner over the takeover), so it stops until Elias rules on the banner and the verdict stamp. **Boards 12a–12f are an Energy track this repo was never asked for** - stopped whole; three of them are beyond the board file's fetch cap. **Nothing new is asked of Design**; the return carries the reading, the outline's re-issue, and thanks. The status that stood here is kept beneath.

## ⭐ STATUS 2026-09-10 — **D19 IS THE LIVE ASK: election night as ONE page around the map, the swing and the government, with the attribution ledger promoted to a readable instrument** (§ ⭐ D19 below). Nothing in it is art. It is asked on real parts: the page was rebuilt the same day (`COMPLETED.md` §437–§439) - the calls stamped with the count at which each landed and collapsed once complete, **board 4a's cartogram drawn at the centre** on our own mandate column (held by `ValkretsCartogramCheck`), the swing like for like against 2022 at every instant, and **who governs** formed on the new chamber with the player's outcome by the office rule. **D18 is closed on both sides** - the fifteen cells landed and the 43 marks are wired (E-31, §429). The package (`docs/outbound/SEND_PACKAGE.md`) carries D19 and the unpasted D17/D18 return in one paste, with **Elias's screenshot of the played night in it by digest** - landed and verified the same day (E-33 closed, §441) - and the nine D16 sitting shots beside it. **D17 is CLOSED (§447, 2026-09-10): Elias ruled the published-hue-plus-lightness-rule palette permanent; item 2's assignment was not needed and is filed as the alternative - the return says so, with thanks. D19 is the only open row.** The status that stood here is kept beneath, demoted.
...
## ⭐ STATUS 2026-09-09 — **D17 IS THE LIVE ASK: the eight party inks, and nothing else — D16's four boards are answered and three of them are BUILT (`COMPLETED.md` §409–§415); board 10d waits on one question about attribution, not about drawing (§ ⭐ D17 below; the package `docs/outbound/SEND_PACKAGE.md` regenerated 2026-09-09).**
...
## ⭐ STATUS 2026-09-05 — **D15 IS THE LIVE ASK: four items from Playtest 5, reconciled against D14 and against Design's own sighting note of 2026-09-04 (§ ⭐ D15 below; the package `docs/outbound/SEND_PACKAGE.md` regenerated 2026-09-05). D14 is answered and built beneath it; its return LANDED AND WAS READ (the board's note "The return of 2026-09-04", nothing asked), so E-15 is done. Sending D15 is Elias's (E-17).** The status that stood here is kept beneath, demoted.

### The D14 status as it stood (2026-09-04, late evening)

## ⭐ STATUS 2026-09-04 (late evening) — **D14 WAS PASTED (E-14, Elias) AND ANSWERED THE SAME EVENING AS BOARDS 8a–8f ON `PoliSim v2 Screens.dc.html`, AND IS BUILT: P6-1…P6-6 in `COMPLETED.md` §305–§310, every board's check clause answered in its row. No ask is live; the RETURN package (`docs/outbound/SEND_PACKAGE.md`, regenerated) is Elias's to paste (E-15).** The status that stood here is kept beneath, verbatim, as the record.

### The D14 status as it stood (2026-09-04, evening)

## ⭐ STATUS 2026-09-04 (evening) — **D14 IS THE LIVE ASK: six items from Playtest 4, reconciled against D12 and D13 (§ ⭐ D14 below; the package `docs/outbound/SEND_PACKAGE.md` regenerated 2026-09-04 evening). D13 was pasted and answered (board 6a) and built; D12 as board 6b; the E-12 return as boards 7a/7b. Sending is Elias's.** The status that stood here is kept beneath, verbatim, as the record.

### The D13 status as it stood (2026-09-03, night)

## ⭐ STATUS 2026-09-03 (night) — **D13 IS THE LIVE ASK: the slider row as a family, reconciled against D12 (§ ⭐ D13 below; the package `docs/outbound/SEND_PACKAGE.md` regenerated). D12 stands beneath it, unsent and unanswered, carried whole. Sending is Elias's.** The status that stood here is kept beneath, verbatim, as the record.

### The D12 status as it stood (2026-09-03)

## ⭐ STATUS 2026-09-03 — **D12 IS THE LIVE ASK: seven rows from Playtest 3, reconciled against D10 and D11 (§ ⭐ D12 below; the package `docs/outbound/SEND_PACKAGE.md` regenerated 2026-09-03). Sending is Elias's.** D11 was pasted, answered and built (§238–§245); the status that stood here is kept beneath, verbatim, as the record.
...
**D10** is Playtest 2's re-composition: nine rows, one annex set (Elias's rows verbatim, the dead-space measure per screen, the films at 1280 / 1600 / 2560), one package (`docs/outbound/SEND_PACKAGE.md`, regenerated 2026-09-02). **Sending is Elias's.** The D9 status that stood here is kept below it, verbatim, as the record.
...
**What that leaves is a RETURN, not an ask** — see `docs/outbound/SEND_PACKAGE.md`: the derived mandate column and the
...
⚠ **SUPERSEDED 2026-09-09 (§419) by D18's generated inventory above, which is the live figure for
every family here.** Two of this section's counts went stale exactly as a transcription does: the marks
read 52 undrawn (the checks now print a smaller number, and it is generated above), and §4 bills two
`StatNodeId` icons that were **already on disk** in the original macro stat pack and resolved the moment
those rows were promoted. **Neither is repaired in place** — a stale fact is re-derived or deleted, never
flipped — so this section stands as the record of what was true on 2026-08-27 and D18 carries what is
true now.
...
⭐ **CLOSED 2026-09-10 (`COMPLETED.md` §447).** Item 1 shipped (§423, 12 of 12). Item 2 was RULED NOT TAKEN: Elias saw the laddered hemicycle and ruled the published-hue-plus-lightness-rule palette permanent; the assignment is filed as the alternative it always was. The section below is kept as the ask stood.
...
1. **The fence, all twelve pairs, measured in oklch on the palette the record holds** — the check is `D16AcceptanceCheck` and the print is in the log named in `docs/outbound/SEND_PACKAGE.md`, by digest. Verbatim:
...
3. **The three built boards, filmed at 1280** — the People page (10a), the statute book and its pane (10b), the Riksbank (10c) — by digest in `docs/outbound/SEND_PACKAGE.md`. Read them rather than this prose: the deviations each board took are stated in `COMPLETED.md` §410, §411 and §412, and the two worth Design's eye are **10b's money cost, refused** (a law in this model carries no fiscal cost at all, so the row's third cell is the approval cost under a head that says so) and **10b's vote bar drawing the desk's own party table** rather than 10d's eight, which is the same reconciliation as this ask.
...
**The ruling this carries** (Elias, 2026-09-14): *"Record with PN-1: the indexed class is two countries, not one, and SFB 2 kap. 10 c § publishes each riktålder six years ahead — so an indexed rule has a short dated path that stops where the law stops. That sharpens 15c's third-mark ask; put both cases to Design in the return (a rule that publishes a window, and one that publishes nothing)."*

### What your boards became

| board | built | what deviates, and why |
|---|---|---|
| **15a** - the energy page composed over the built page | `COMPLETED.md` §486: the rule row one device with the parts under their names (a zero struck, the cut dashed, scarcity in Caution), the wholesale row its head and the re-derivation its foot, the two retail stacks on one scale, the chain's links in the gutters with BINDS as a mark and a word, the water value under the blocks, the two ledgers as 13b's bridge twice on one scale with one leader, the absent rows in place; filmed in six countries at two widths | four, stated there: the legend line in the plate's foot; the REACHES chips in PROVENANCE as 9c has them; the illustrative Δ tick not built; the head and the body drawn as two rows of one plate |
| **15b** - the schedule row | `COMPLETED.md` §490: under the income tax's own row the marginal rate over income as a curve in the track column (the statute solid, the draft dashed, the thresholds ticked, y 0–50 on every row), one sub-row per band named by its threshold span with a dial on the rate alone, Germany's § 32a as ramps whose end rates are derived from its coefficients (the ramps' integral within a euro of the formula at every zone end), France's parts axis and the CSG line, Poland's levy and the USA's deduction as their own lines, Italy's layers BILLED; the sub-rows' figures ride the budget bill | four, stated there: the lever's figure stays the line's rate with the average effective rate at the mean income as its second line (the lever is what the political scale reads); Sweden's state brackets drawn sourced, not dashed (the model has the file your paste lacked); the draft curve redrawn from the statute as built; the × PARTS chip is the kind word PER PART, the parts one per person |
| **15c** - the statutory mark | `COMPLETED.md` §491 and §493: under the pension line a D13 row whose knob is the age in force this year (60–70), the law's path as the tick sprite at every future statutory value with its year ABOVE the track, the knob joined to the next step by a dotted hairline, the kind as 9c's chip under the name, NEXT and NO LEVER in the band, the citation's short form under the figure | three: the override state is not drawn - the pensions driver is deferred by ruling, so no bill sets an age yet and the row says NO LEVER; the paragraph verbatim stands on the Policy Web node, the short citation on the row (the verbatim overran the trailing cell in four countries); the years sit above the track (under it they printed over the band) |

### The third mark — two cases, asked back

Your 15c drew an indexed rule *dated only to its horizon, the third mark a date with no figure*. The record now says exactly what the two indexed statutes publish, and the calendar reaches both ends of the case:

| case | the statutes | what the law publishes ahead | on the row today | the question |
|---|---|---|---|---|
| **A - a rule that publishes a WINDOW** | Sweden: Socialförsäkringsbalken (2010:110) 2 kap. 10 a–10 c §§ - a year's riktålder applies from the sixth year after its calculation. Italy: decreto-legge 201/2011 art. 24 - the ISTAT adjustment every two years | a short dated path that stops where the law stops: Sweden 67 through 2032, then 2033 a date with no figure; Italy 67 through 2026, 67 y 1 m in 2027, 67 y 3 m in 2028, then 2029 a date with no figure | the path's ticks up to the horizon; past it a half-tint tick beyond the last known age labelled "2033 ?"; the band NEXT 2033 · NO FIGURE YET · NO LEVER (Sweden at 1280, annexed) | how a window reads against a schedule's end on the same grammar - Germany's path ends in 2031 and France's in 2033 because their laws CLOSE there, where Sweden's and Italy's merely stop being published: the same last tick, a different fact |
| **B - a rule that publishes NOTHING for the year in force** | the same two statutes once the calendar has passed the window - Italy from 2029, which is the films' own year; Sweden from 2033 | no figure for this year or any year after. The model computes no life expectancy, so the age in force is the last published figure, CARRIED | built structurally (§493), because the row had printed the horizon's year as NEXT once the calendar passed it: the knob at the carried figure (67 y 3 m), a half-tint tick labelled with THIS year ("2029 ?"), the band CARRIED FROM 2028 · NO LEVER (Italy at 1280 and 2560, annexed) | how a knob that is itself a carried figure reads beside a knob that is the law's own - and whether the row draws any path at all when nothing ahead is known |

**What is not asked:** the ages and the rule kinds (sourced, asserted by `PensionAgeDiagnostic`); a glyph (still a costed follow-up, as D20 said); the override state (it waits for the driver, by ruling).

### Carried, not re-asked

D19 item 1 (stopped on this side on three citations; the banner ruled here by DS-8); D-7 (board 2b); D8-2; D8-4 and D8-5 (behind K-1); D8-6; D-E4 (in E-3's return); `sweden_outline.svg` (held - re-issue with a digest that matches its manifest).

### The annex

- **15c:** the pension row at 1280 for Sweden (case A), Italy (case B), Germany (a schedule) and Poland (a fixed rule), and Italy at 2560 - `sp2_<country>_<width>_05b_budget_spending_pension.png`.
- **15b:** the tax tab scrolled to the schedule rows at 1280 for the six - `b15b_<country>_1280_05a_budget_tax_rows.png`.
- **15a:** the energy page's three frames at 1280 for Sweden and Germany - `b15a_<country>_1280_06c_policylaws_sectors_energy{,_mid,_lower}.png`.
- **The statutes:** `ElectionsData/pensions/README.md` and `statutory_ages.csv`.

## ⭐ D20 — THE ENERGY PAGE AS A BOARD OVER A BUILT PAGE, THE SCHEDULE ROW'S GRAMMAR ON THE SOURCED SHAPES, THE STATUTORY MARK ON A LEVER THE LAW ALSO MOVES — one ask, three batches, and the reconciliation that says what is not asked (2026-09-12)

**The rulings this carries** (`COMPLETED.md` §474, Elias): DS-4 - *"Structural ship first in the v3 grammar, the board second, read the D11 way against a built page. The D19 way worked: real parts make a better ask than a description does."* DS-4b - *"the Sectors page now, the rail cell asked in D20 and taken only if the page proves it needs one."* DS-5 - *"Open the energy ask on our side: yes. Recording 12a–12f's four revisions as my instructions: no — I did not give them, and I do not record instructions I did not give. The boards are Design's answer to our D19 and are legitimate as such: read them the D11 way, and any board asserting an instruction the record does not hold stops that board, not the set."* And, on assembling this ask: *"the rule row's two forms (merit order and reservoir dispatch, with the row saying which priced the block) is a composition question Design must see, not a detail; and the absent rows are content, not gaps to hide — the page's honesty is part of what's being composed."*

### The reconciliation — nothing is asked twice

| what stands | its state on this side | what follows for this ask |
|---|---|---|
| **D12 row 1** - the stylized six-country map | **BUILT** (§268–§274) | the energy prototype's foot deferred it *"until D12 row 1's map lands"* - the condition is satisfied, or it named the wrong map (a Sweden zone map is D8-3's kind). **Instrument-first stands (§452): no map is asked.** |
| **D13** - the slider family | **BUILT** (§267) | inherited by the pension dial row (batch 3) and the schedule row (batch 2); no new row form is asked |
| **D19 item 2** - the ledger as an instrument | **BUILT** (§450) on board 13b | its painter is reused by this page's two ledger bridges (batch 1); nothing re-asked |
| **D19 item 1** - election night as one page | **STOPPED on this side** (§449) on three citations; **DS-8 rules the conflict** (§474): *"keep B8's banner; a text chip from primitives now, D-8.5's art later"* | **carried open, not re-asked** - the three citations are in the notes back below; the board builds the day the banner question is closed between us, and it is closed this way on this side |
| **Boards 12a–12f** - Design's energy track | **read the D11 way in this pass, board by board - the table under the head below** | a board's composition is taken where it verifies against the built page; a board asserting an instruction the record does not hold stops - that board, not the set. **No instruction is recorded as Elias's**: the four revisions the boards quote are not in this repo's record, and it does not record what it was not given |
| **D-7** (board 2b), **D8-2**, **D8-4/5** (behind K-1, 13 September), **D8-6**, **D-E4** | open, as at D18 | carried, not re-asked |
| **D17, D18** | closed (§447, §429) | nothing |

### Boards 12a–12f, read the D11 way against the built page

**How they were read.** `PoliSim v2 Screens.dc.html` fetched live on 2026-09-12 (`DesignSync get_file`, the 256 KiB cap as always). The file has moved since §449's fetch of 2026-09-10: **a new board, `14a`, sits at the top** (drawn 2026-09-11 from the spec-let, before anything was built), and the cut now falls inside `13b` - so `energy-note-2` and `12c` are beyond today's cap. The region from `12f` to `13b` is byte-identical between the two fetches (202 111 bytes compared), so §449's reading of `12f` and `12e` stands as content; `12c` and `energy-note-2` are cited from the 2026-09-10 fetch, said so. `12a`, `12b` and `12d` were beyond the cap on both fetches and are not read. Every figure below was checked against what this repo holds on 2026-09-12: the energy spine and its catalog (`EnergyData/`), the built page, the records.

| board | present | rests on | verified against the repo | not held, or not verifiable | verdict |
|---|---|---|---|---|---|
| **14a** - *"THE ENERGY SCREEN — THE RIKSBANK's SHAPE AT A LARGER SCALE, WITH THE TWO LEDGERS AS THE THESIS INSTRUMENT"* (2026-09-11, *"drawn before anything is built · 1149×691 pinned · every figure illustrative, every form derived"*) | **whole, live** | the spec-let as it stood (*"reference-ruled and instrument-first"* - §454 and §452, held) and *"docs/specs/ENERGY_SPEC.md read whole"* - a document this repo holds only as the spec-let derived from it (E-34's open question, where the source is, may be answered by that file's existence on Design's side). **No instruction attributed to Elias.** | **the forms match the page as built** to a degree that makes this ask a reconciliation: THE TWO LEDGERS as two bars of one length with Σ = Σ drawn, not footnoted (the page: two bridges on 13b's painter, the close the book's own gap); THE CLEARING as three blocks with the marginal unit NAMED under its price (the page: "COAL SETS IT", per block); THE ZONES as *"a small multiple whose cell count is data - Sweden four, the others one each with ZONE STRUCTURE UNSOURCED"* (the page: Sweden's four, one elsewhere, Italy's seven and the USA's three stated); *"a price prints in Caution only where its cut binds"* (the page: BINDS on the link, Caution on a scarcity block); instrument-first, map-first refused, 12f's outline waiting on D12 row 1 (**D12 row 1 is built, §268–§274 - the condition the board waits on is satisfied or misnamed**); the retail waterfall as 10c's; the DECLARED line present and honest | *"12c's statute book and dial strip survive untouched at Sectors › Energy"* - **no energy statute book exists in the game** (EN-7 is a row: one law category, unbuilt); the five dials are five dial rows, not a 24 px strip. *"THE MINISTRY … Needs stage 7"* - **the spec-let has no stage 7** and the game no energy minister. *"MARGIN UNDER THE 10 % RULE"* - **no adequacy rule exists**; a dependable-vs-peak margin is derivable from the clearing (`DependableMw` per block) but nothing rules 10 %. *"CAPITAL RECOVERY 4.1"* in the system-cost bar - **the book has no capital recovery**: its system cost is fuel and O&M, ETS and adders, and the inframarginal rent is PRINTED, not modelled. *"14 g CO₂⁄kWh INTENSITY DERIVED · P5-C5's ROW"* - derivable (the clearing's CO₂ over its GWh), **not drawn today**. *"SETS: NUCLEAR / CHP / GAS PEAKER"* - the game's marginal units are coal, gas and oil, and **Sweden's zones are priced by the reservoirs, not a merit order** - the page's second form, which the board never saw. Every figure illustrative by its own line. | **READ, and taken as Design's answer to the spec-let's board ask** - it asserts no instruction the record does not hold. **Batch 1 is now a reconciliation of 14a against the built page**, not a composition from a description: what matches is taken, what the page holds and 14a lacks is asked, what 14a holds and the game does not is said and not asked. |
| **12f** - Energy r4, *"Sweden as its outline"* (2026-09-10 night) | whole, identical in both fetches | *`Elias: "I don't like this stylized map of sweden straight up with just less details"`* - **not in this repo's record** (DS-5: not recorded as his) | the outline's projection centroid (*"Flataklocken, the centroid docs/data/COUNTRY_CENTROIDS.md already holds"*: 62.3875 N, held); 7a is a read board (§283); the sites *"all public"*; *"Capacities aggregated and ILLUSTRATIVE"*, *"cuts drawn as parallels … declared approximate"*, *"load ∝ population as the declared proxy"* - declared, consistent | the hydro-cluster GW (Lule 4.2 …) are not the spine's fleet figures and say so; `sweden_outline.svg` **held** (§448: fails its own manifest); *"the ruling is revised: the cartogram is the Sweden of seats; the outline is the Sweden of places"* - a revision of 13b's note 2 this side has not taken (§452: instrument-first stands, no map) | **STOPS** as a board - it rests on an instruction the record does not hold. Its rules (*"an overlay changes ink and lines, never the geometry; one ink; Caution for constraint and verdict"*) are Design's own and are read as such. |
| **12e** - Energy r3, *"three overlays on one geometry"* (2026-09-10) | whole, identical in both fetches | *"Elias's three crops read: Svenska kraftnät's grid · Behovskartan's demand hexes · a project-pin map"* and *"GEOMETRY SUPERSEDED BY 12f (Elias, same night: not the cartogram)"* - **not in the record** | *"the four areas exist, the three cuts exist, the six interconnectors exist"* - the spine holds SE1–SE4, the chain's three links and ten external links (`external_links_se.csv`); SNITT 1 3 300 and SNITT 2 7 300 are the NTC figures the catalog carries; *"13b's note 2 ruled one Sweden"* held (§449); 4a built; *"All figures illustrative; the substrate holds none of them"* declared | **SNITT 4 5 400 is not the spine's 5 600** (§452: where they differ the spine's stand); the six interconnector chips (NO 3 700 · FI 2 400 · LT 700 · DK 2 400 · DE 600 · PL 600) do not match the spine's ten links (Norway 3 995 out, Finland 2 700, Denmark 2 015 out and 2 415 in, Germany 615, Lithuania 700) and **the spine holds no Poland link** - the board says they are *"public figures quoted from memory, ILLUSTRATIVE until sourced"*, and they are; the prices 41 · 42 · 68 · 91 öre illustrative; *"no load by region in the game"* true, said | **STOPS** as a board. Its admission rule (*"an overlay may change ink; it may not change where Sweden is"*) and *"a price splits only where a lane is full"* are Design's own grammar, read as such. |
| **12c** - Energy r2, *"two maps and a statute book"* (2026-09-10, *"Elias's redirect"*) | truncated in the 2026-09-10 fetch, beyond the cap today | the redirect (`energy-note-2`, below) | *"10c's waterfall unchanged"* - the device the built rule row draws; the hero *"0.56 t CO₂ ⁄ PERSON · EDGAR 2023"* is the environment family's own figure (§349); *"ILLUSTRATIVE MIX"*, *"ABSENT · NAMED FETCH"* declared | the statute book's seven acts (*Carbon tax act REACHES THE MODEL +$2.4B · Electricity certificate act · Nuclear phase-out act CAPS N1–N6 · Grid concession act · Energy tax act +$6.1B · Offshore permitting act · Feed-in premium act*) - **none is a law this game holds**; the amendment diff - **no mechanic**; *"4a's eleven bands re-weighted by megawatts"* - **no MW by valkrets band exists in the repo** (the layer's zones are ENTSO-E's); *"136 öre HOUSEHOLD"* against the book's 27.2 ¢ USD - illustrative by its own line | **STOPS** as a board - the redirect it rests on is not in the record. |
| **energy-note-2** - *"the redirect read"* (2026-09-10) | whole in the 2026-09-10 fetch, beyond the cap today | *`Elias: "maps, less direct sliders and more enacting and changing existing laws."`* - **the instruction itself, not in the record** | *"4a's valkrets cartogram is declared geometry, eleven bands and twenty-nine tiles"* built (§438); *"10b already ruled the statute book"* built (§411) | *"an amendment diff … the in-force value as a standing tick and the drafted value as a pencil knob"* - **no law in this game carries a parameter to amend**; *"The five dials survive as a 24 px strip with law-set end stops"* - not the game | **STOPS** - it is the instruction the record does not hold, verbatim. |
| **12a, 12b, 12d** - r1's page and mechanics, r2's geometry | **absent from both fetches** (beyond the cap) | 12a *"partly superseded by r2"* by the masthead; 12b *"the stack's geometry and degradation ladder, and how a state project appears over time"*; 12d *"two faults found in 7a's projection method"* | nothing - not read | 12b's degradation ladder and state project name mechanics the layer states ABSENT by design (no investment, no retirement) | **NOT READ** - unverifiable by content; each stops on that alone. |

**What the reading yields for this ask.** One board, 14a, is Design's answer to the spec-let's own board ask and is read whole: batch 1 below is its reconciliation against the built page. Five boards and the note stop on the instruction each rests on - as DS-5 ruled, that board and not the set - and their own grammar (overlay = ink only; Caution where a cut binds; the cell count as data) is read back as Design's, which 14a already carries into the instrument-first screen. **No instruction is recorded as Elias's.** Two figures are corrected for the spine (snitt 4 at 5 600, no Poland link), one held file is re-asked (`sweden_outline.svg`), and one question on Design's side is noted for E-34 (the source document's whereabouts).

### What is asked — three batches, largest first

| n of 3 | batch | what it inherits | what is genuinely new | what it unblocks |
|---|---|---|---|---|
| **1 of 3** | **the energy page, composed** - over the page as built and filmed (annex, twelve frames) | the 9c/10a plate rows, the `Distribution` stacked bar, the 10c waterfall, the small-multiple strip, the 13b bridge | **the two ledgers as the page's own device**, and **the rule row's two forms** | the page's second pass; stage 6 as amended |
| **2 of 3** | **the schedule row** - a new instrument class on the sourced shapes (annex, the tariffs verbatim) | the tax `LedgerRow`, the D13 slider, the 10c waterfall | **the row itself** - no grammar exists for a tariff | F4-4, the row on the screen after F4-2's schedules |
| **3 of 3** | **the statutory mark** - a lever the law also moves, on the pension dial row | the D13 dial row, the carbon caption's sentence, 9c's plate | **only the mark** | PN-1's screen (S5), PN-2's caption |

#### 1 of 3 — the energy page: 14a reconciled against what is built

The page is built and filmed: four plates under Policy › Economic Sectors, in the plate grammar, one family. **Your 14a already drew the instrument-first screen from the spec-let, before the build** - and read the D11 way (the table above) its forms match the page's: the two ledgers as one length with Σ = Σ drawn, the clearing with the marginal unit named, the zones as a small multiple whose cell count is data, Caution only where a cut binds. So batch 1 is not a composition from a description; it is the reconciliation of 14a against the built page, in three parts. **Taken**, where the board and the page agree: the ledgers' device (the page draws them on 13b's painter), the marginal unit named per block, the honest cell count, the retail waterfall as 10c's. **Asked**, where the page holds what 14a never saw: the rule on the price in its TWO forms (below - the ruling's first addition), the absent rows as content (the ruling's second), the water value and the reservoir balance as Sweden's rows, the congestion rent, the wholesale price against the other five's own clearings, and the page's placement as a section of the Sectors page rather than a screen of its own. **Said, and not asked**: 14a's statute book and dial strip (no energy law exists in the game - EN-7 is a row), its ministry (no stage 7, no energy minister), its 10 % adequacy rule (no rule; the margin is derivable), its capital-recovery bar (the book prints the inframarginal rent and models no capital), its intensity row (derivable, not drawn), and its merit stack of nuclear · CHP · peaker (the game's marginal units are coal, gas and oil, and Sweden's zones are not priced by a merit order at all). Every part the page draws is real and named here so the board is drawn over parts:

| part | what it draws today | the composition question |
|---|---|---|
| the two retail stacks | six parts per kWh in cents (wholesale · margin · network · levies · environmental tax · VAT), the book's figure beside | one row per class, or the two classes as one device |
| the wholesale price | load-weighted over the blocks, this country's tick against the other five's own clearings this turn | as drawn, or nearer the rule below |
| **the rule on the price - TWO FORMS** | per block a waterfall: for Germany, France, Italy, Poland and the USA the marginal fossil unit's fuel and O&M, its ETS cost, its calibration adder and the tranche-or-scarcity term above its mean offer; **for Sweden's zones the reservoir dispatch's parts - the seed's zone price carried by the level, the water value's departure from its seed proxy on the zone's beta, the deficit slope - and the cut where a binding link splits the chain.** The row says which form priced the block ("COAL SETS IT · A CHEAPER TRANCHE", "THE RESERVOIRS PRICE IT", "CURTAILED") | **⚠ the ruling's first addition: this is a composition question Design must see, not a detail.** Two countries price by two rules on one page; the board decides whether the two forms share one device with the form named, or are two devices - and how the marginal unit and the cut are read at a glance |
| the fleet | capacity by technology beside this year's dispatched generation, two stacked bars; wind and solar utilisation as chips | as drawn, or one bar with two readings |
| the zones | Sweden's four as a small multiple - three block prices and loads per zone, the chain's three links' peak flows against their capacities, BINDS where a block fills one; every other country its one zone, with the deviation stated (Italy's seven, the USA's three interconnections) | the strip's form; whether a link's BINDS is a mark or a word |
| the water and the reservoirs (Sweden) | the water value per block, the balance against the store with its sparkline, the congestion rent | a section of Sweden's page, not a map (§452) |
| **the two ledgers as bridges** | system cost: fuel and O&M + ETS + adders, then the inframarginal rent, closing on the wholesale outlay; incidence: households, non-households, taxpayers paid; generators, suppliers, networks, support, the state received; the close the book's own gap - on 13b's painter over its geometry | **the page's own device** - the thesis of the layer, which has no grammar yet; 13b's bridge is the nearest, and this is where it is either enough or not |
| the instruments | the ETS price with NO PATH (the 2023 mean carried by the price level; absent for the USA), the energy line's levy scale (absent for Germany - the climate fund sits off the budget), the carbon tax that REACHES TRANSPORT, NOT THE FLEET, the sector dials DESCRIPTIVE UNTIL THEIR STAGE MAPS THEM | reached-by chips as drawn, or a strip of their own |
| **the absent rows** | investment and retirement (no rule - the fleet is the seed's), load growth (the load does not grow with GDP or electrification), the hydro fold billed for France, Poland and the USA, Italy's seven zones and the USA's three, each an ABSENT row with its reason in the row | **⚠ the ruling's second addition: these are content, not gaps to hide - the page's honesty is part of what is being composed.** The board composes them as it composes the figures |

**The rail cell (DS-4b).** The page is a section of the Sectors page today and needs no icon. **Asked as a question, not declared:** does the page prove it needs a cell of its own on the rail? If the board says yes, `icon_area_energy` becomes a real asset row in the next ask, cut small-size-first like the six, and only then is the area declared (declaring it first would fail the area-icon check red with nothing to draw). If no, the page stays where it is.

**What is NOT asked in this batch.** No map (§452; a Sweden zone map is D8-3's kind and D8-3 is open elsewhere). No figure: every number on the page is the state's book or the market's own clearing, and the annex frames carry them. No new hue. Nothing that draws a quantity the layer does not hold - the absent rows are the honest surface for those, and they are content.

#### 2 of 3 — the schedule row: one grammar, six honest shapes

F4-2 (the plan's S3a) puts a SCHEDULE on every income-tax line - flat today; a bracket table for Poland, the USA and Sweden's state layer; Germany's §32a formula, verbatim; France's barème applied per part; Italy's three layers billed until its statute lands (E-6). F4-4 then puts the schedule on the screen as a row. **No grammar exists for a tariff**; the question is asked on the SOURCED SHAPES, annexed verbatim from each country's own authority (`ElectionsData/tax/README.md` and the files beside it): how a schedule reads as a row of the tax ledger; how a player moves a bracket's RATE and not its threshold (the spec-let's S4); how a formula (Germany), a barème per part (France) and three layers (Italy) draw honestly beside three bracket tables - one grammar, six shapes; the average effective rate as the row's one figure, the per-bracket rates as sub-rows of the D13 slider family. **The one dial** stays one dial: it shifts every rate uniformly (DS-2c); the per-bracket rates are the row's own.

**What is NOT asked:** the tariffs themselves (sourced), a new slider (D13's), a new hue.

#### 3 of 3 — the statutory mark: a lever the law also moves

PN-1 (the plan's S5) builds the pension age as `CarbonRateStatute`'s precedent: one rule per country with its paragraph (Sweden's riktålder by life expectancy, SGB VI § 35 and § 235's birth-year table, CSS L161-17-2's schedule to 64, DL 201/2011 art. 24, Poland fixed, 42 U.S.C. 416(l)) - the statute moves the figure at the boundary before any override, a passed bill's figure wins, and the caption says *"Between decisions the rate moves as the law moves it"* as the carbon row already does. **Asked: the one distinction the row needs** - the law's path beside the player's figure, which stands this year - on the D13 dial row and 9c's plate. A MOVES BY STATUTE glyph is a costed follow-up, not an asset gap; the sentence is the delivered form until a board proves it short. The statutes are annexed (`ElectionsData/pensions/README.md`, `statutory_ages.csv` with each statute's own sentence).

**What is NOT asked:** the ages (sourced), the dial (D13's), the plate (9c's).

### What is NOT asked, and why — the reconciliation, generated

⚠ **Nothing is asked twice, and a batch that unblocks nothing is not requested.** The coverage checks' reading is the generated block under D18 (the same block, rebuilt on every bar; `D18InventoryCheck` fails the build when it and the repo disagree):

| family | the checks' reading | disposition |
|---|---|---|
| **Stat icons** | every `StatNodeId` name resolves, **0 GAPs** | **NOT ASKED.** The plan's S-E1 row expected the energy figures declared as stat nodes so the inventory would print their icon gaps. On inspection the page's rows draw no icon - the plate grammar has no icon slot - and a stat node's icon is drawn only on the Policy screen's rail and the web, which the energy figures do not join (no coupling reaches them from a policy node). **Declaring nodes for figures nothing draws an icon for would be queued art in another costume** (F1's rule); nothing is declared, nothing is asked. |
| **Area icons** | every `SystemArea` member resolves | **NOT ASKED - a question instead** (batch 1, the rail cell): declared only if the board proves the page needs a cell. |
| **Portraits** (four families) · **Audio cues** · **the delivered-and-held set** | as at D18 | **NOT ASKED**, unchanged. |

### Carried from the earlier asks — open, and NOT re-asked here

| row | what it is | state |
|---|---|---|
| **D19 item 1** | election night as ONE page | **stopped on this side on three citations the record does not hold** (§449, the notes back below); **the conflict is ruled on this side** (DS-8): B8's banner stays over the takeover; the verdict is a text chip from primitives now, D-8.5's art later. The composition is largely buildable and builds the day the citations are withdrawn or replaced |
| **D-7** | board 2b, the Policy Web drawn to be read | open since D7 |
| **D8-2** | five countries' party colours - a ruling, not art | open |
| **D8-4, D8-5** | election night's paper, the verdict stamp | open, behind K-1 (13 September 2026) |
| **D8-6** | modal or stage | open |
| **D-E4** | the two Society icons | in E-3's return |
| `sweden_outline.svg` | board 12f's geometry | **held** (§448): it fails its own manifest - re-issue with a digest that matches, with its board |

### Constraints — standing, restated

- **Delivered sprites and primitives until a board proves a gap.** Every part of the energy page draws from the plate grammar and 13b's painter; nothing here asks for a second way to draw what the desk already draws.
- **No new hue without a costed case.** The page inks itself in the Sectors area's colour.
- **Every figure is the state's own or the market's own clearing.** A board that prints a figure the page does not carry is a board asking for a rule, not a drawing - and a rule is asked as a row here, never drawn into existence.
- **The absent rows are content** (the ruling's addition): a board that hides them composes a different page than the one built.
- **No instruction is recorded that was not given.** The 12a–12f boards' quoted revisions are not in this repo's record; where a board rests on one, that board stops and the rest are read.
- **Binary artifacts are uploaded, not described** - the twelve frames in the annex are files with digests, and nothing here summarises in prose what a `get_file` can read back.

### The annex — what goes with this ask

- **The energy page, filmed** - twelve frames, the page's top, middle and lower half for **Sweden** (four zones, the reservoir form of the rule, the chain's links, the water and the reservoirs) and **Germany** (one zone, the merit-order form with coal setting every block from a cheaper tranche, the levy row ABSENT because the climate fund sits off the budget) at 1280 and 2560: `en6e_<country>_<width>_06c_policylaws_sectors_energy{,_mid,_lower}.png`, digests in `docs/outbound/SEND_PACKAGE.md`.
- **The sourced tariff shapes** for batch 2: `ElectionsData/tax/README.md` with the statutes' own pages beside it (Germany's §32a formula, France's barème per part, Poland's brackets, the USA's four tables; Italy billed, E-6).
- **The statutes** for batch 3: `ElectionsData/pensions/README.md` and `statutory_ages.csv` - one rule per country with the statute's own sentence.
- **The notes back**, carried whole beneath in the package: D19 item 1's three citations as §449 read them, the B8 ruling (DS-8), the outline's re-issue, and that D12 row 1 is built.

### The inventory — the checks' own print

The generated block is D18's, below, rebuilt on every bar; this ask adds no family to it and the block says so: stat icons 0 GAPs, area icons complete, marks 53 of 53.
...
- **Elias's screenshot of the played night, verbatim - the picture that caused this ask**: `Elias Screenshots/shots/2026-09-10_election_night.png`
  (1804 × 889, SHA-256 `707663df9fb5d264…`; verified against its manifest, by signature and by CRC walk, §441). A played night at 29 of
  29: nine call rows every one "at 29 of 29", the constituencies as raw counts, "no previous election is on hand", and no government.
...
- **The films after items 1-3**, 1280 and 2560: the board in its five frames (the model's night, and the staged night early,
  partial, called, final) and the played night's takeover. Named by digest in `docs/outbound/SEND_PACKAGE.md`.
...
⚠ **Read this before any figure elsewhere in this document.** Every count below is EMITTED by
`D18Inventory.Emit`, which runs the coverage checks in-process and writes their own lines here; the
splice is `Tools/d18_inventory.sh`, one command, and `D18InventoryCheck` fails the build when this
block and the repo disagree. **Nothing here is typed, so nothing here can quietly go stale** — which
is not a hypothetical: section 0 of this document, hand-derived on 2026-08-27, still bills two icons
that were delivered in the first sprite pack and counts nine more undrawn marks than exist.

<!-- GENERATED by D18Inventory.Emit (Tools/d18_inventory.sh). DO NOT EDIT BY HAND. source-digest: c0bd8ca3db0ab8b0ba5dae8af9230cd4fe3a5899a478a3c5ecbd30d13778660c -->
*Every figure in this section is EMITTED, never typed: `D18Inventory.Emit` runs the coverage
checks in-process and writes their own lines here. Regenerate with `Tools/d18_inventory.sh`.
`D18InventoryCheck` runs in the cheap bar and fails it when this block and the repo disagree,
so a figure here cannot go stale without the build going red.*

#### 1 · Party identity marks — `PartyMarkCoverageCheck`

```
  permanent mark_party_us_lib - ⚠ ARITHMETIC, not an omission. `PartySystem` seeds the USA by SEATS in the House, whose SOURCED size is 435 - and it seeds REP 220 + DEM 215 = 435, the WHOLE chamber. There is no seat for a third party to hold, so a Libertarian party cannot be seeded without taking a seat from one of the two that hold them, which would be inventing a result. The file is kept rather than retired: it is delivered art, and the day this model gains a vote-share dimension the USA's third parties live in, it is already drawn.
=== Party marks: 53 seeded part(ies), 53 with a resolving mark, 0 without one, 0 delivered-but-unconsumed (ceiling 0), 0 error(s) ===
```

**By chamber** (seeded parties, the marks that resolve, and what is left to cut; the rule
`mark_party_<iso2>_<abbrev>` re-derived against all 53 delivered marks on this run):

| country | chamber seats | parties seeded | marks resolving | to cut |
|---|--:|--:|--:|--:|
| USA | 435 | 2 | 2 | 0 |
| Sweden | 349 | 8 | 8 | 0 |
| Germany | 630 | 9 | 9 | 0 |
...
chamber shows most; the stem is the file the game will load once the seed names it:

| country | abbrev | party | seats at seed | stem |
|---|---|---|--:|---|

#### 2 · Portraits — `PortraitCoverageCheck`

```
  sitting   USA                  Harriet Ellsworth        no portrait (none requested; not counted)
  sitting   Sweden               Ingrid Hallstrom         no portrait (none requested; not counted)
  sitting   Poland               Marek Zawadzki           no portrait (none requested; not counted)
=== Portrait coverage: 25 of 25 pool members resolve through Resources.Load (18 ministers across 6 portfolios + 7 Fed chairs; 3 sitting chair(s) reported, not counted; NOT Portraits/ on disk) ===
```

#### 3 · Stat icons — `StatIconCoverageCheck`

```
=== UI art coverage: 21 of 21 names resolve, 0 reported GAP(s) (every StatNodeId icon + menu_pattern_tile; NOT chrome, emblems, marks or portraits) ===
```

#### 4 · Area icons — `AreaIconCoverageCheck`

```
=== Area icons: 11 of 11 resolve (SystemArea members as RGBA32), 0 missing, 0 damaged ===
```

#### 5 · Audio cues — `AudioCueCoverageCheck`

```
=== AudioCueCoverageCheck (P4-2): the cue catalog against the files, both directions ===
    cue                    file             state        note
    ButtonPress            Click1           ok           the pack's first click
    SliderStep             Click2           stand-in     the pack's Slider_Click_Sound is an AAC stream in an MP4 container, which Unity's importer does not read; SET ASIDE by ruling (E-10, Elias 2026-09-03) - the second click stands in, and stays a stand-in
    FolderSwitch           Folderswitch1    ok           the pack's first folder switch
    FolderReturn           Folderswitch2    ok           the pack's second folder switch
    BillPasses             ApprovalStamp    ok           the pack's approval stamp
    BillFails              -                NO FILE      the pack's RejectedBillTear is an AAC stream in an MP4 container, which Unity's importer does not read; SET ASIDE by ruling (E-10, Elias 2026-09-03) - no stand-in, a stamp or a click would say the wrong thing; the cue is wired and silent
    InterruptRaised        Folderswitch2    stand-in     the pack holds no interrupt sound; a folder landing on the Desk stands in, and stays a stand-in (E-10, closed by ruling 2026-09-03)
    ConstituencyDeclares   Click2           stand-in     the pack holds no declaration sound; a click stands in, and stays a stand-in (E-10, closed by ruling 2026-09-03)
    8 cue(s), 5 file(s) under Assets/Resources/Audio/Cues, 5 named by a cue.
    3 stand-in(s) [SliderStep (Click2), InterruptRaised (Folderswitch2), ConstituencyDeclares (Click2)] and 1 cue(s) without a file [BillFails] - reported as such by ruling (E-10, 2026-09-03), not accepted as covered.
=== AudioCueCoverageCheck: ALL ASSERTIONS PASS ===
```

#### 6 · Delivered and held — nothing in `Assets/Scripts/` can load these

*198 sprite(s) under `Assets/Resources/Art/UI/`. A file counts as REACHED when an
accessor the game calls actually loads it on this run (the texture's own name is taken as the
key, so no naming rule is duplicated here), or when a source file names its stem as a whole
token. The rest are below: delivered work with no way in. **They are ours to wire, not
Design's to cut again.***

| stem | folder |
|---|---|
| `ui_frame_double` | Chrome |
| `ui_portrait_frame_oval` | Chrome |
| `ui_stamp_draft` | Chrome |
| `emblem_party_centristcoalition` | Emblems |
| `emblem_party_conservativeunion` | Emblems |
| `emblem_party_nationalistfront` | Emblems |
| `emblem_party_progressivealliance` | Emblems |
| `mark_party_us_lib` | Emblems |
| `badge_revised` | Stats |
| `icon_release_marker` | Stats |
| `icon_stat_birthrate` | Stats |
| `icon_stat_budget` | Stats |
| `icon_stat_consumption` | Stats |
| `icon_stat_creditrating` | Stats |
| `icon_stat_currencystrength` | Stats |
| `icon_stat_deathrate` | Stats |
| `icon_stat_governmentdebt` | Stats |
| `icon_stat_homeownership` | Stats |
| `icon_stat_housepriceindex` | Stats |
| `icon_stat_housingcost` | Stats |
| `icon_stat_inequality` | Stats |
| `icon_stat_inflationexpectations` | Stats |
| `icon_stat_investment` | Stats |
| `icon_stat_netmigrationrate` | Stats |
| `icon_stat_population` | Stats |
| `icon_stat_productivity` | Stats |
| `icon_stat_realwages` | Stats |
| `icon_trend_down` | Stats |
| `icon_trend_flat` | Stats |

29 held, 169 reached.

**The runs behind this block**: PartyMark 0 · Portrait 0 · StatIcon 0 · AreaIcon 0 · AudioCue 0 — 0 is a clean check; a non-zero code means that family's own bar is red and this
inventory is describing a broken build.
<!-- END GENERATED -->
...
⚠ **D14 (below) was pasted, answered as boards 8a–8f and BUILT (`COMPLETED.md` §305–§310); its return landed and was read (the board's note "The return of 2026-09-04", nothing asked). D15 asks nothing twice.** It is what Playtest 5 (2026-09-05, `POLISIM_FEATURE_LIST.md` § Playtest 5, Tracks A–D) needs drawn, and only that. Design has already SIGHTED the engineering files of this playtest (the board's note "Playtest 5 sighted before D15 — not a paste", 2026-09-04 night) and wrote what it will read for when the ask lands; this ask is written against that note, item by item, so nothing is described that the note already read.

**Reconciliation.** D14 item 1 (board 8a, the pie and bar as instruments) is built (§309) and then CHANGED by P5-A1 (§313): the leaders are gone. The sighting note accepts that in advance ("a leader that crosses says less than no leader"); item 1 below states the deviation once and asks for the read-back, not a redraw. D13 (board 6a, the slider row as a family) is built (§267) and P5-B5 (§316) re-purposes its row for a figure with instruments in the caption band; the note says "it will want a board" - item 2 is that board. Items 3 and 4 are grammar for what Track C will build (P5-C2–C7, not built; the note names 5c and 8a as the likely parents) - asked now so the first family lands on a drawn grammar rather than an improvised one.

### The four — n of 4

| n of 4 | item | what exists | what is asked | the constraint that binds it | evidence on disk |
|---|---|---|---|---|---|
| **1 of 4** | **The pie without leaders — a deviation from 8a, stated** | `PieChartRenderer` after §313: wedges from 12 o'clock largest-first, paper hairlines between, hatched OTHER past the eighth series, the legend beside the disc keyed by swatch (one swatch, one name, one share per line), wedges of 40° and more printing their share inside at 0.62 r in paper ink, the rest reading off the legend; no leader anywhere. The disc holds 8a's 120 px. The Playtest 5 finding it answers: with eight wedges the leaders converged on one column and two crossed. | **The read-back only**: that 8a stands with its leader rule struck - the legend keyed by swatch and the ≥40° inside label ARE the label rule now. If Design would rather the inside threshold sit elsewhere than 40° (the smallest wedge that can carry "12.3 %" at 1280 in paper ink), say the angle. | 8a's other rules unchanged: ≤8 inks, no figure moves, the legend's order is the wedge order. | `p5a_1280_04a_demographics_pie.png`, `p5a_2560_04a_demographics_pie.png` (the People page after §313), `p5al_1280_ladder_pie.png` (nine synthetic series: eight wedges and a hatched OTHER, the legend to nine lines), by digest below. |
| **2 of 4** | **The Budget line row for a figure — 6a's row re-purposed** | `GameController.DrawSpendingLineRow` after §314–§316: the standing tick is the line's amount and the knob the drafted FIGURE; the track is this year's allowed change around the standing amount (±30 % Discretionary, ±15 % Mandatory), the figure cell prints the standing amount (the drafted one in the draft cue when drafted), the trailing cell the share of GDP. The caption band's two ends carry the instruments - left the driver's short name and the ratio the last index applied (`65+ ×1.013`, `JOBLESS RATE ×0.957`, `NO DRIVER`, `PINNED`), right `Δ +$585M · NEXT $44.6B` (the delta against the year-open amount and the projection, the same arithmetic the turn runs) - 35 % of the band each at rest, 20 % while a range caption (8d) is painted, the ratio and then the delta dropped when the end is too narrow (measured, never clipped). The book is in constant prices (§314): there is no price term and no price instrument. | **The row drawn as 6a's family member**: where the driver and NEXT sit when the band is also 8d's caption band (the two ends are the engineering answer - is the board's?); the delta's place (the trailing cell was tried and collided with the share under the figure); what a PINNED row looks like at rest (a pin glyph? the knob's face? the tick?); and how a row reads when its driver has moved against the player (the jobless line falling while the player raised it - two directions on one track). | 6a's row geometry is fixed (P4-1's guard: no column rect moves between rest and drag - 110 rows asserted at both widths); the caption band's height is 8d's; ≤8 inks; the figure never abbreviates. | `p5b5_1280_05b_budget_spending_rows.png`, `_rows_dragged.png`, `_dragged.png` and the four at 2560; the film logs; by digest below. |
| **3 of 4** | **One instrument grammar for the society stats, family by family** | Nothing built: P5-C2 (health: coverage, quality, wait times, retiree coverage), C3 (education: PISA score, graduation, attainment), C4 (infrastructure: road condition, congestion, …), C5 (environment: CO₂ per capita split, …), C6 (immigration and poverty depth) are sheeted as one BASELINE family per session, each "seeded for six, coupled, displayed as instruments, family explained". The People page (8a) and the effect plates (5c) exist as the parents the note names. | **The grammar, not the screens**: one drawing that says how a society-stat family is presented wherever it lands - a figure with its unit and source line, its sparkline or band, its coupling arrows (5c's) to the dials that reach it, and its place (the Statistics page? the People page? the policy page of the ministry that funds it?) - drawn once on health (C2, the first family) so C3–C6 inherit it. If a family's stats are three figures and a distribution, say which instrument each takes. | The stat-plate grammar of 8a and the plate rules of 5c (≤8 inks, the arrow renderer, the caption mono for source lines); a family arrives with its OECD/Eurostat source named - the source line is part of the instrument. | The C2 row's text (`POLISIM_FEATURE_LIST.md` § Playtest 5, Track C); the current People page (`p5a_2560_04_demographics.png`) and a 5c plate (`p6a_2560_03a_decisions_options.png`, in the D14 return) as the parents. |
| **4 of 4** | **The Cabinet's effectiveness readout** | Nothing built: P5-C7 sheets effectiveness = allocated ÷ requested, scaled by the minister's efficiency attribute (P2-5), where the request is B2's driver-indexed line and the allocation the player's figure; "the Cabinet and Budget screens show it". The Cabinet page draws each portfolio's minister card (portrait, attributes) today; the Budget row (item 2) is where the allocation is set. | **Where and how the readout sits**: on the minister's card (a figure beside the efficiency attribute? a band?) and on the Budget row (a third instrument in the band? a mark on the figure cell?), and how "underfunded" reads at a glance without a colour that is not in the ink ladder - the mechanic is a ratio of two money figures times an attribute, and the readout must not become a second book of any of the three. | The minister card's geometry (P2-5, the portrait frame and attribute rows) and the Budget row's (item 2); ≤8 inks; a ratio prints as a ratio, never as a score. | The Cabinet page film (`p5b4_1280_07c_politics_cabinet.png` if in the sweep - the fund-page sweep stopped before it; the D14 return's `p6a_*` sets hold the Docket, not the Cabinet), the C7 row's text. |
...
4. **`docs/generated/BUDGET_PREMISE.md`** - the generated liveness audit (93 live, 0 dead, 33 not armable with the reason per row, §315); Design is not asked to read it. **`COMPLETED.md` §312–§316** for the records; §314 carries the constant-price finding and the sixty-year table.

**Binary artifacts are uploaded, not described.** Nothing in this ask summarises in prose what a `get_file` can read back.

---

## ⭐ THE ONE LIVE ASK — D14, six items from Playtest 4, reconciled against D12 and D13 (2026-09-04)

⚠ **D13 (below) was pasted and answered as board 6a and BUILT (`COMPLETED.md` §267); D12's seven rows as board 6b, six built and one ruled (§268–§274); the E-12 return was answered as boards 7a and 7b and built (§282–§283). D14 asks nothing twice.** It is what Playtest 4 (2026-09-04, `POLISIM_FEATURE_LIST.md` § Playtest 4) found on the built screens after its Tracks B and E changed them, and it is written AGAINST THE FILMS of those changes, named per row, at 1280 and 2560. Rows are numbered *n of 6* so a partial answer is legible.

**Reconciliation.** D12 row 1 (the stylized map) is built as 6b delivered it - six chips at geographic centres - and D14 item 5 asks only to CONFIRM the chip set now that the grid under it is gone (P4-E1, §296), not to redraw the map. D12 row 3 (the trade graph, the delta convention) was answered by 6b row 3 as the dotted zero rule and built (P5-4); D14 item 2 asks the graph's COMPOSITION - the head row, the axis labels, the pager - which no board has drawn, and carries the film that shows the rule in place (P4-E4, §299). D12 row 6 (the Riksbank 720 fold) stands under its ruling; D14 item 3 is the page AFTER P4-E2's cut of its restating captions (§297), which changed what is on it. D13 (the slider family) is built as 6a; D14 item 4 is the range CAPTION under that family - a thing 6a did not have to draw because it did not exist until P4-B1/B2 (§288–§289). D10 row 2 asked the Docket option's cost and arrows and D14 item 6 is that ask made structural (P4-E3, §298): the card exists in the 5c grammar and needs its face. Item 1 is new: the People page's pie and bar instruments have never been on a board.

### The six — n of 6

| n of 6 | item | what exists | what is asked | the constraint that binds it | evidence on disk |
|---|---|---|---|---|---|
| **1 of 6** | **The People page's pie and bar instruments** | Two pies by `PieChartRenderer` (the dependency ratio; employment by sector) and three bar instruments (population by five-year band with the working-age and old-age rules; the dependency tri-band; the electorate's thirteen voter groups in one ink with hairline breaks), every figure the model's or SCB's and stamped DERIVED / SOURCED as it is. | **The pie and the bar drawn as instruments of the paper idiom** - the pie's wedge and its label, the bar's face, the stamp's place - one grammar the page's five instruments obey. | The palette caps categorical series at eight inks (`UiPalette.MaxCategoricalSeries`); nothing draws more series than inks. The provenance stamps (DERIVED, SOURCED · SCB 2014) stay on every instrument, in the caption mono. No figure moves. | `p4g_1280_04_demographics` / `_deep`, and the same at 2560 |
| **2 of 6** | **The trade graph's composition** | The trade balance graph (Statistics · International) draws by `GraphRenderer` at board 1l's weights, with 6b row 3's dotted zero rule in place (P5-4; verified P4-E4, §299) and the money delta from a zero base (Δ −$14.7B). Its head row is the renderer's furniture: the title, the delta, the Older/Newer pager, the axis labels laid over the plot's left edge. | **The composition of the graph as one instrument** - where the head's figure and delta sit, where the axis labels sit against the plot, what the pager looks like in the idiom. | The rule at zero stays dotted and stays at zero; the delta stays the money delta (the ±100 % form is retired, P3-C4); the plot's height is the sheet's fraction the renderer already takes. | `p4g_1280_02b_statistics_international_deep` and at 2560 |
| **3 of 6** | **The Riksbank page after the cut** — extends D12 row 6 | The page after P4-E2 (§297): the lead figure, the projected path with TODAY / +1 YR / +2 YR, the rule's five terms with their operators, the three reading lanes, the governor. The four restating captions are gone; the graph's head title is gone and its delta reads in points, or not at all when the window is flat. | **The page's composition after the cut** - whether the space the captions held closes up (the instruments move up) or is given to the instruments (the path taller, the lanes wider), and the 720 fold under D12 row 6's ruling. | The order of the page stands (the rate, the rule, the readings, the governor); every term on it is the code's own and none is paraphrased. | `p4g_1280_07d_politics_federalreserve` (before: `p4c_1280_07d_politics_federalreserve`) and at 2560 |
| **4 of 6** | **The range caption's presentation** — under D13's family | Every dial (fourteen) carries ten authored bands (P4-B1, §288); on drag the band's name and line appear beneath the slider in the row's own reserved band, hold 1.5 s and fade over 2.5 s (P4-B2, §289), lower-centred under the track, in the caption mono. | **Placement, type and fade, drawn**: where under the slider the caption sits against the end-names, its face and size, whether the name and the line share a line, and the fade's curve. | The caption never reflows the row (the P4-1 invariant, asserted on film); it lives inside the band the row already reserves; the hold and fade timings are the harness's clock and may change if the drawing asks. | `p4b4_1280_06a_policylaws_labormarket_caption_drag` / `_held` / `_faded` and the Manufacturing subsidy trio on `06c`; the same at 2560 |
| **5 of 6** | **The map on plain paper** — extends D12 row 1 | 6b's stylized map is built: six chips at geographic centres, the links between them, on the Desk and on Statistics; the grid that sat under it is removed (P4-E1, §296). | **Confirm the chip set on plain paper** - that the six chips as delivered read without the grid, or a ruling on what the paper carries instead (a rule, a border, nothing). | The chips and their snap pitch stay; the map's rect stays; one texture serves both maps. | `p4g_1280_01c_desk`, `p4g_1280_02b_statistics_international` and at 2560 |
| **6 of 6** | **The Docket option card with cost and arrows** — D10 row 2, structural now | Every option under a Cabinet decision draws the cost line in the Budget's words and ink (*Cost of this option +$X*), the plate caption (*ESTIMATED IMPACT · THIS OPTION*), the effect arrows by the one renderer, and the option's own scope line (P4-E3, §298). | **The option card's face** - how the button, the cost line and the plate compose as one card, and how two or three options stack on the Docket without the page reading as a column of plates. | The arrows are the renderer's (5c) and are not redrawn per card; the cost line's ink is the Budget's (Bad for a cost, Good for a saving); below the ministry's knowledge floor the card says it cannot estimate and shows no plate. | `p4g_1280_03a_decisions_options` (two options visible) and at 2560 |
...
4. **The lever map** - `docs/generated/LEVER_MAP.md` (P4-C1, §291), generated, for the record of what the model reaches; Design is not asked to read it.
...
**Reconciliation, row by row.** D10 rows 1 (International), 3 (People) and 8 (the frame) are extended here by D11 rows 1, 2 and 4; D10 rows 6 and 7 (the two result screens) are folded into D11 row 3 as one family with the effects panel; D10 row 5's Riksbank panel is given its page by D11 row 6; D9 rows 2–5 (marks and inks) become the status row, D11 row 5. D10 rows 2 (Docket), 4 (Laws) and 9 (contrast) stand as written and are not repeated.

### The rows — 6 of 6

| n of 6 | row | what exists | what is asked | the constraint that binds it | evidence on disk |
|---|---|---|---|---|---|
| **1 of 6** | **Statistics › International** — *extends D10 row 1* | The country pages are built structurally (P-E1, `COMPLETED.md` §102): prev/next browsing over the other five, both sides' headline readings side by side, the pair's trade in both directions from the map's own `TradePartner` links, the tariff each charges through the simulation's own `GetTariffRate`, shared bloc and shared currency, and both sides' policy-stance readings (`PolicyStanceAxes` - the two blends the compass plotted until P2-3.2; the CHES compass positions are not on the page). | The composition that makes a pair's page read as **one modern page rather than a stack**: how the two sides sit, what leads, how absence is drawn. | **Only what the model holds.** Every gap named on the page in its own words - *no bilateral relations state at all*; *no trade link* drawn as a different fact from *trade of zero*. No relations score, no invented affinity, no warm/cool reading the model has no data for. | `p2mn<width>_02b_statistics_international.png` (and `_deep`, `_rows`) at 1280 and 2560; the dead-space figure per capture in `p2an1280.log` / `p2an2560.log` |
| **2 of 6** | **People** — *extends D10 row 3* | The cohort substrate exists (F2): `PopulationCohorts` - twenty-one five-year bands, the open band last - derives the total, any age range, the old-age and total dependency ratios, the elderly and school-age shares; `CohortDemographics` walks the bands to each year's target; `CohortVoterGroups` derives the voting age (sourced per constitution), the eligible population, the voter groups' population shares and, for Sweden, turnout by age (SCB 2014, the series' end). The People screen today is a rolling list: a dependency pie, a sector-employment pie, two ledgers, a population bulletin and a population-share pie. | People as **instruments over real cohorts**: the pyramid, participation by band, dependency as the substrate derives it, the electorate's shape (the eligible bands and what votes). | **Derived only** - each quantity named above, none authored. **The eight-series cap** (`UiPalette.CategoricalSeries` holds eight inks; nothing draws more series than inks). **Honesty classes visible** - DERIVED / DECLARED / SOURCED / MEASURED printed on the instrument, as the coalition page already prints them. | `p2mn<width>_04_demographics.png` at 1280 and 2560; the quantity names are the code's (`PopulationCohorts`, `CohortVoterGroups`, `CohortDemographics`), read from it, not transcribed |
| **3 of 6** | **The effects page — one family with the two result screens** — *new; folds D10 rows 6 and 7* | P2-2.1 retired the four horizons and built the arrows panel: one arrow per outcome the draft moves next year, length by the move's size relative to the largest, ink by whether the move is good for that outcome (`UiPalette.GetDeltaColor`, direction-aware), the figure at the head, lanes sized by their widest word (`EffectArrowsRenderer`). P2-4.3 gave the signing ceremony and election night their full-frame content: the vote as seats, the citation, the estimate as the same arrows painted once (`CanvasPaint`), the campaign's attribution ledger beside the count. | The effects panel and the two result screens drawn as **one family with one visual grammar for "estimated impact"** - the arrow, the figure, the scope line - on the sheet and on the takeover paper. | **No false precision.** The preview is deterministic: one point with its scope stated ("next year, with vs without this draft, no events"); a range that is not a range is forbidden. The arrow's length is relative to the largest move in the panel, never to an absolute scale. | at rest and mid-draft: `p2mn<width>_05r_budget_rest.png`, `05b_budget_spending.png`; the ceremony `p2mn<width>_89e_signing_settled.png`; election night `p2di<width>_e7c_election_night_takeover.png` (the interrupts pass) - all at 1280 and 2560 |
| **4 of 6** | **Folder tabs on the left — board 1n-r3** — *extends D10 row 8 (the frame)* | The rail is icon cells (1n-r2): a cell is derived, not chosen - the icon is the tab font size times `ConsolidatedTabIconFontMultiple`, the cell is the icon scaled by `(RailIconGridUnits + 2 × RailMarginUnits) / RailIconGridUnits` on the 24-unit grid, the glyph, pad and caption gap are fixed fractions of the cell (`RailGlyphSize`, `RailCellPad`, `RailCaptionGap`), so the four widths give four cell sizes from one rule. The HOME cell and the PAUSE/RUN chip sit on it; the active tab carries a spine in its area's accent; the sheet meets the rail with no gap (P2-1.1). The v2 frame's tongues retired with the OPEN state (D-set §1). | **Board 1n-r3**: the left edge read as **folder tabs** - the v2 tongue idiom brought to the one-frame rail - at the derived cell sizes, the HOME cell and the PAUSE/RUN chip preserved, the active-tab spine preserved, flush to the sheet. | **No new sprite assumed.** Tongues from the existing chrome primitives (the sliced paper, the hairline, the accent wash) until a board proves a gap; if it does, the gap is named as a costed row, not drawn into the ask. | every `p2mn<width>_*` capture carries the rail; the derivation is `GameController.RailCellWidth` and its three fractions |
| **5 of 6** | **Colours and logos for parties — status, not a new ask** — *D9 rows 2–5* | Elias's GO was given (E-2, 2026-09-01); seven Swedish marks are cut with S re-cut, and the fifty-cell batch with its four assignment rules exists on Design's side. Every call site, coverage check and fallback (the hairline *no published colour* swatch) is built and waiting. D8-2's ink collision still stands: S and V draw the same hex (S-7), and six of Sweden's eight inks sit inside the derived legibility floor (S-8); Playtest 2 added the finding that the four right-bloc inks read as one blue at dot size on the per-seat hemicycle (§227). | **The batch delivered as files** - binary artifacts uploaded, not described - and **a ruling on the ink collision**: S against V, and the right bloc's four blues. | **The election authority's published hues are the starting point** (Valmyndigheten's `fargkod`, the desk seating derived from it); any departure is Design's to make and to state on the board. | `PartyInkHarness`'s current measurements, printed by the run, not transcribed: `d11_partyink.log`; the hemicycle at dot size `p2mn<width>_07a_politics_parliament.png` |
| **6 of 6** | **The Riksbank page** — *new; gives D10 row 5's panel its page* | P2-3.4 built the model's own content (§230): the rate today; the rule's inputs as readings - inflation against `TaylorRule.InflationTarget`, unemployment against NAIRU (`GetUnemploymentGapPercent`), the output gap (`GetOutputGapPercent`, a reading the rule does not weigh); the rule's reading (`GetSuggestedInterestRate` - the neutral real rate, inflation, the inflation gap at `InflationGapWeight`, the unemployment gap at `UnemploymentGapWeight`); the governor's lean and target; and the projected path (`RatePathProjection`): today, after this year's move toward the rule on today's readings, after next year's toward the rule on the preview's readings, the governor closing `RateAdjustmentSpeed` of the gap each year. The rate's history draws as a graph beneath. | **The page that presents it**: the rate's history and the projection as **one graph at board 1l's weights** (history three pixels over the projection's two-pixel dashes); the inputs as instruments; the governor and the appointment lever as the page's political half. | **The projected path is the rule evaluated on the projection and says so** - next year's readings are the deterministic preview's, nothing beyond the preview's year is forecast, and no forecast is dressed as a fact. | `p2mn<width>_07d_politics_federalreserve.png` (and `_deep`, `_rows`) at 1280 and 2560; the figures are the code's (`TaylorRule`, `RatePathProjection`, `PolicyPreview`), referenced, not transcribed |

### The annex — one set

1. **The films** at 1280×720 and 2560×1440 on the tree that closes Playtest 2 (`p2mn<width>_*`, the sweep; `p2di<width>_*`, the interrupts pass for election night), listed in the package by name, size and digest.
2. **The dead-space figure per capture** - the `dead nn.n%` column in `p2an1280.log` and `p2an2560.log` (P2-4.1's measure; the same stems).
3. **The party-ink measurements** - `d11_partyink.log`, `PartyInkHarness`'s own print: the derived floor and every sourced hue's distance from every area accent.
4. **The derivations named**, for rows 2, 4 and 6 - `PopulationCohorts` / `CohortVoterGroups` / `CohortDemographics`; `GameController.RailCellWidth` and its fractions; `TaylorRule` / `RatePathProjection` / `PolicyPreview` - read from the code, never restated as numbers here.
5. **Elias's rows, verbatim** - `POLISIM_FEATURE_LIST.md` § P2, as D10's annex carried them; the complaint is still the brief.

**Binary artifacts are uploaded, not described.** Nothing in this ask summarises in prose what a `get_file` can read back.

---

## D10 — Playtest 2's re-composition (2026-09-02, P2-4.2) — UNSENT; carried by D11 above, row for row (the reconciliation at its head)
...
|---|---|---|---|
| **1 of 9** | **International — modernise and re-compose** — *extended by D11 row 1* | The country pages exist structurally (C-C8): a trade partner's page says what the model holds and, in its own words, what it does not — *no bilateral relations state at all*, and *"no trade link exists between these two"* drawn as a different fact from *"trade of zero"*. Every gap the model cannot fill is named on the page. The board composes what is there; it does not draw a warm/cool reading the model has no data for. | `p2an<width>_02b_statistics_international.png` (and `_deep`, `_rows`) at the three widths |
