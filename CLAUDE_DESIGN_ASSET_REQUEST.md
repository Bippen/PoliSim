...
## ⭐ STATUS 2026-09-18 — **D21 IS THE LIVE ASK, IN TWO PARTS: asked now, the Energy area's icon and ink (the rail cell that stands with a tinted E), three forms the People page's measurement says it needs, and the pie's size and the bar it falls back to; ARRIVING, the Energy tab's composition - the tab exists, is reachable and holds your D20 plates, its first decision (build and retire) is on it since this pass, and the composition ask follows against the built page the D19 way, not against the spec-let's text. Reconciled against D20; nothing answered is re-asked.** (§ ⭐ D21 below.) The rows are Playtest 6's (`COMPLETED.md` §526–§539). Sending stays Elias's (E-41): one archive, `AssetPackArchive/D21_ask.zip`, cut with `MANIFEST.sha256` inside and verified both ways.

## ⭐ D21 — THE ASK (2026-09-18), IN TWO PARTS
...
### Part 2 — ARRIVING: the Energy tab's composition
...
- **Its first decision is on it.** Build and retire (§539): a line per technology with the fleet it has and its lead time (EIA AEO2023 Table 1: coal 4 y, gas 3, nuclear 6, wind 3, solar 2), a step down and a step up, the connection queue with each order's year placed and year it connects or leaves, and the cost row reading **BILLED** - IRENA's 2024 costs are the CAPEX source and they are owed, so nothing is priced and the page says so. Filmed at rest and with one order placed (`filmf2c_1280_08_energy_decisions`, `filmf2c_1280_08_energy_decisions_ordered`).
- **What follows as its own items:** the four policy instruments as the dials S9 named, the law category's reach from the tab, the ministry on the finance ministry's pattern.
- **The composition ask follows the D19 way** - against the built page with the decisions on it, real parts, not against the spec-let's text. Until then nothing in Part 2 is asked. The page as it stands is in the archive with its ABSENT rows honest: `Load growth · ABSENT · THE LOAD DOES NOT GROW WITH GDP OR ELECTRIFICATION · STATED, NOT MODELLED`, and the *Investment and retirement* row that read ABSENT since EN-6 is retired, because the decision exists.
