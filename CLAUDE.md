...
| `specs/` | `ENERGY_SPEC.md`, `TAX_SPEC.md`, `docs/specs/ELECTIONS_CAMPAIGN_SPEC.md`, and since §613 `POLITICAL_SYSTEM_SPEC.md` and `START_POINTS_AND_PARTY_CREATION_SPEC.md` (its §9 the main menu and settings) — the source specs; the head track of `POLISIM_FEATURE_LIST.md` is their work list |
...
- **THE WORLD CLOCK** (ruled standing 2026-09-25, PS-1, §618): one world on one calendar whose start is the chosen country's (`WorldClock.StartDate`) - a scheduled election starts at the standard run-up before its polling day, a snap election on the first day a primary source records that an early election was set in motion (Germany 2024-11-06, Italy 2022-07-21), France in governing mode from 2024-07-18 with no election modelled. **The chamber of record is seated AS ELECTED - the election's lists** (mid-term clubs are a recorded deviation, never seated), and the government of record governs where its cabinet is sourced (`SeatedGovernment.TryAt`); a chamber whose per-list table is not on disk seats the latest sourced one and its view says so (E-47). `ElectionVintage.Seated` resolves at the epoch; a check pins an election by name. The epoch is set before a world is created and never while one lives; a save carries its own. **Editor code that opens a world on a start or loads a save runs under `SimulationManager.EpochScope()`** - the process epoch between checks is the default, and a leak moves every check after it (the bar found one, §618). **The election is a DATE** (PS-2 / CL-4, §619): the player's country votes on its own real polling day (`WorldClock.TryNextPollingDay`, the statute's rule, sourced) inside whatever turn it falls in - `SimulationManager.PollingDayToday` is the one gate, `ElectionSystem.IsElectionTurn` times no election; a country whose calendar is not modelled holds none and its chamber stands as of record; the formation after an election reads that election's declarations (`WorldClock.VintageOfElection`); losing office never ends a run, leaving the chamber does. **Declarations by date** (ruled standing 2026-09-25, §621): a declaration is dated by the party's own record, never by press reporting; a document is dated by the decision it records, not its file date; a declaration stands until a later dated one replaces it (`DeclaredRedLines.SwedenTimeline`, `ForDate`; the vintage API stays for the backtests). **One seed vintage per country, the seed's** (§621): economies are never re-seeded per start and never mixed within a country - the offset to the calendar is stated (`WorldClock.SeedVintageLine`). **Italy seats the list as filed** (§621); a joint list without its own CHES position takes the seat-weighted mean of its members' positions, stated on the row. Every election on a country's calendar inside a run is simulated once that country's model exists; until then its chamber and head of state hold as of record.
- **Declarations are dated, and every election reads its own date's** (ruled standing 2026-09-24, K-1f, §607). A party's declared red line, prime-ministerial candidacy or in-or-against rule is a dated fact with its citation (`ElectionsData/sweden/coalition_declarations_2022.md`, `ElectionsData/sweden/2026/coalition_declarations_2026.md`). A formation reads the ruled declarations as they stood at ITS election: the 2022 backtests read 2022's (`ElectionVintage.Sweden2022`), and the seated chamber reads 2026's (`Seated`). The formation wires only what a ruling named. Declarations on record that no ruling named (MP's and SD's in-or-against, KD's refusal of Andersson; K-1g) stay dated facts in the files and are not read by the formation. An in-game election after 2026 has no dated set of its own, so it reads 2026's until a newer set is sourced; that carry-forward is the implementation's default, not the ruling's words. A lifted declaration is removed, not carried.
...
- **The document checks run OUT of the engine** (§574): `Tools/textcheck` compiles the same source files under shims and answers five of the eight in ~1.2 s, against a 23 s document bar. It is a loop instrument, not the bar - except one mode: **since RL-1 (§588) it carries the review ledger's comment-only verdict** (Roslyn), which the cheap bar builds on demand with `dotnet`; a verdict it cannot reach is never an acceptance.
