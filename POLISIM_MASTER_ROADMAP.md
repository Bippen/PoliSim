...
5. **Political Systems Overhaul — Part B, full rollout** — REVISED (2026-07-31): not a uniform per-tab repeat of the pilot, but a three-tier bill design (annual omnibus budget, standalone program add/remove, standalone non-budget policy) built in six sub-phases, 5a through 5f. Only once step 4 is validated (it is). **5a, 5b, 5c, and 5d DONE (2026-07-31); 5e IN PROGRESS (Phases A and B DONE, Phase C batches 1 — the tab bar — and 2 — card chrome + Decisions — DONE, all live-confirmed; Phase C batches 3+ per-tab content restyling remain; scope now combined with 5f).** Full spec in Part B below.
...
## Part B — Parliament (MASTER SEQUENCE STEPS 4 and 5; step 4 DONE, step 5 plan REVISED 2026-07-31, 5a/5b/5c/5d DONE, 5e IN PROGRESS - Phases A and B DONE, Phase C batches 1 and 2 DONE, batches 3+ remain, scope now combined with 5f)
...
**Batches 3+ — the remaining tab content: NOT STARTED.** Left to restyle: **Demographics**, **Politics**
(Parliament / Compass / Cabinet / Federal Reserve), **Policy/Laws** (5 sub-categories), and
**Tax/Spending** (the shared Budget Process screen). 2-3 at a time, live-confirmed per batch. Suggested
next batch: **Politics + Demographics** — `PoliSimWidgets.SupportBar` is purpose-built for Parliament's
seat/majority readout, and the **real Cabinet and Fed Chair portrait art is imported but still entirely
unused** (`PoliSimWidgets.Portrait` is a procedural placeholder that draws no art asset at all). Note
those portraits live under `Assets/Art/UI/Portraits`, NOT under `Assets/Resources/` - so wiring them up
needs the same move the icons needed (see `IconLibrary`'s own doc comment for why `Resources.Load` and
not `AssetDatabase`). Budget Process is the natural LAST batch: it is the biggest screen and
`StandingDraftPair`/`DraftTrack` were designed for its line items specifically.
