...
## Part B — Parliament (MASTER SEQUENCE STEPS 4 and 5; step 4 DONE, step 5 plan REVISED 2026-07-31, 5a/5b/5c/5d DONE, 5e IN PROGRESS - Phases A and B DONE, Phase C batches 1-3 DONE and live-confirmed, batches 4-5 built and awaiting confirmation, scope now combined with 5f)
...
**Remaining work — recommended next batch:** the one genuinely valuable thing `StandingDraftPair` would
have provided is its amber "draft differs from standing" cue, which the pack's own documentation calls
the only signal that a change is pending a vote and therefore "never optional". That can be delivered
without any of the fragile geometry, by coloring the existing draft labels with `PoliSimTheme.Draft`
when the draft diverges — no layout math, width-adaptive, one call per site. There are **~21 such
`Standing: X, Draft: Y` label sites** across Crime & Justice, Labor, Trade, Tax and the remaining budget
categories. Do it as its own batch and all at once: applying it to some screens but not others is worse
than not doing it, because an absent amber cue would then wrongly imply "nothing changed here".
