...
5. **Political Systems Overhaul — Part B, full rollout** — REVISED (2026-07-31): not a uniform per-tab repeat of the pilot, but a three-tier bill design (annual omnibus budget, standalone program add/remove, standalone non-budget policy) built in six sub-phases, 5a through 5f. Only once step 4 is validated (it is). **5a, 5b, 5c, and 5d DONE (2026-07-31); 5e IN PROGRESS (assets imported, three-phase plan written, Phase A tab-placement calls awaiting Elias's confirmation; scope now combined with 5f).** Full spec in Part B below.
...
## Part B — Parliament (MASTER SEQUENCE STEPS 4 and 5; step 4 DONE, step 5 plan REVISED 2026-07-31, 5a/5b/5c/5d DONE, 5e IN PROGRESS - assets imported, three-phase plan written, Phase A tab-placement calls open, scope now combined with 5f)
...
#### Phase A — Tab/IA restructuring (no visual style changes yet)

Reuses the existing tab-bar UI mechanics (color-coded per area, same interaction pattern, same
`DrawRightColumnTabButton`) - only the grouping/navigation changes in this phase, not the visual style
(icons/sprites are Phase B/C's job).
...
**Old → new mapping - GENUINELY AMBIGUOUS, not directly stated in the original 5e scope text - escalated
here rather than assumed (see Open Questions below for Elias's decision):**

| Old tab | Recommended new tab | Reasoning |
|---|---|---|
| `Trade` | SPLIT: Statistics + Policy/Laws | Mixed content, unlike every confirmed row above: the Trade Balance graph and per-partner import/export volume bars are informational (Statistics-flavored), but the tariff bill status/live-estimate/Introduce action (Master Sequence step 5d's `TradePolicyBill`, a genuine standalone bill) is exactly what "Policy/Laws (standalone bills from 5d)" describes. Recommend splitting the same way Cabinet does above. |
| `FederalReserve` | Politics | Not named in the original 5e text at all. Recommended because it's a real political institution with its own active lever (interest rate, Fed Chair selection) that groups naturally with Parliament/Cabinet, even though the Fed/Eurozone exemption means it's never Parliament-gated. |
| `PolicyWeb` | Statistics | Not named in the original 5e text at all. Recommended because it's a cross-cutting visualization (like World Map) tying together ~73 nodes across every policy area, not a policy-setting screen. |
| `Infrastructure` | Tax or Spending (same screen) | Not named in the original 5e text at all. Recommended for consistency with Welfare/SWF above - `DrawInfrastructureContent` is ALREADY reused verbatim inside Budget Process's own Infrastructure category, and the standalone tab has no independent lever of its own (its own doc comment: *"Condition Index... driven by the Infrastructure spending category... not a dial here"*), so it's arguably not even a genuinely separate case, just the last one to fold in. |
| Does "Decisions" need the Budget Process mandatory-pause interrupt too? | Recommend yes | The Tax/Spending destination screen owns the ONGOING Budget Process UI (status + Introduce), but "Decisions (pending... bill-vote interrupts)" suggests the ANNUAL budget process's own mandatory-pause moment (the one truly blocking bill-related interrupt that exists today) should ALSO surface in Decisions when active, mirroring how Foreign Policy/Cabinet's modals do - not fully resolved by the table above, since it's a "does one screen's status ALSO appear in a second screen" question, not a straightforward single-destination mapping. |

**Validation for Phase A**: single-scenario smoke check (pure navigation, no economic logic touched) PLUS
a manual click-through of all 7 new tabs, confirming every consolidated system's content actually
renders and nothing got lost in the move. Report back with a screenshot of the new tab bar and reconfirm
the full old→new mapping (including the escalated rows, once Elias has answered them) before Phase B
starts.
...
- **OPEN (2026-07-31) — Phase A tab-placement calls, escalated per this document's own "escalate genuine design forks rather than guessing" rule.** See Part B's "5e implementation plan" section above for the full reasoning behind each; summarized here for tracking. Recommendations given, none yet confirmed by Elias:
  1. `Trade` tab - split its informational content (Trade Balance graph, partner volume bars) into Statistics, and its policy content (the `TradePolicyBill` status/estimate/Introduce action) into Policy/Laws.
  2. `FederalReserve` tab - recommend Politics (a political institution with its own lever, groups with Parliament/Cabinet), not named in the original 5e scope text.
  3. `PolicyWeb` tab - recommend Statistics (a cross-cutting visualization, not a policy-setting screen), not named in the original 5e scope text.
  4. `Infrastructure` tab - recommend folding into the Tax/Spending (Budget Process) destination alongside Welfare/SWF, since its content is already reused there and it has no independent lever of its own.
  5. Should the Budget Process mandatory-pause interrupt ALSO surface under Decisions (mirroring Foreign Policy/Cabinet's modals), even though the ongoing Budget Process UI itself lives under Tax/Spending? Recommend yes, for consistency.
