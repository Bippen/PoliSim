...
| **Playtest 1, finding 1 — meta-text on player surfaces** (Elias, 2026-08-29, the Sweden session; no screenshot — the words are the finding) | the finding's record; **P-A1** answers it | the laws tab on any current family (`we1_campaign_*_…` is the newest sweep; the "COMPLETED" text is on the Policy/Laws › Laws sheet); the cut's own film when P-A1 runs | Elias, verbatim in substance: *developer-facing text is leaking into player surfaces — "COMPLETED" in the laws tab, progress markers, anything addressed to the builder rather than the player.* P-A1: a census of every UI string against a banned-token list, every artifact cut, `MetaTextCheck` armed. The row closes on the census table and the filmed cuts |
| **Playtest 1, finding 2 — the "as published" graph block** (Elias, 2026-08-29) | the finding's record; **P-A2** answers it | `*_02a_statistics_domestic_deep` on the current family (the published-series graphs at the bottom of Statistics); the cut's own film | Elias, verbatim in substance: *the "as published" graphs at the bottom of Statistics are redundant.* A DISPLAY cut — `PublicationSystem` and its PRELIMINARY/revision conventions stay where the main graphs already carry them; a harness line proves §19's perceived-performance still reads the published series |
...
|---|---|---|---|
| 1 | *Developer-facing text is leaking into player surfaces — "COMPLETED" in the laws tab, progress markers, anything addressed to the builder rather than the player.* | **P-A1** — the census against a banned-token list, every artifact cut, `MetaTextCheck` armed so the class cannot silently return | a session's tail; earlier is better because it improves every screen the elections track films after it (also a §V row) |
| 2 | *The "as published" graph block at the bottom of Statistics is redundant.* | **P-A2** — a DISPLAY cut only; `PublicationSystem` untouched (§19 perceived-performance reads Published, never State), PRELIMINARY/revision honesty stays on the main graphs; a harness line proves the election model still reads the published series | a session's tail (also a §V row) |
| 3 | *Every tax/spending draft change should show its estimated annual fiscal impact before enactment — revenue delta, spending delta, net — as a range, never false precision.* | **P-B1** — the law browser's estimate idiom on the Budget and Tax/Welfare surfaces, the range's width the model's own projection uncertainty | a session's tail |
| 4 | *Entering office should open the budget process immediately for the first fiscal year — the player lays a budget on arrival instead of waiting for the calendar's next cycle.* | **P-B2** — the incoming-government budget window (Sweden's own practice) as calendar data per country; the seeded budget the starting point; the no-policy baseline byte-identical (the window must not move it) | a session's tail |
