...
| Stale register text | F-1 "CampaignRun still UNWIRED, held by C-R4b" - wired since §205–§207 (the trigger "a campaign the player actually runs" is satisfiable, unobserved); the shelf's "(Rosatellum.cs, TacticalVoting.cs)" gloss - `TacticalVoting` is reached (D-10 (a)); the second unwired file is `ElectoralCollege.cs`. Corrected in S1 | `UnwiredSubsystemCheck.cs`, `NationalElection.cs` |
