# The answered Design asks - PoliSim

> **Moved here VERBATIM from `CLAUDE_DESIGN_ASSET_REQUEST.md` on 2026-09-23** (`COMPLETED.md` §582), in the order they stood there. Nothing below was rewritten; one bracketed line marks where the generated D18 block stayed behind. The live ask is `CLAUDE_DESIGN_ASSET_REQUEST.md`. **A figure or a path below is a record of the day it was written**, not a claim about today: re-derive before trusting it, as the live file's count rule says.
> The request rows here are still read by `DesignNotificationCheck`: a row tagged [CUT] must be named in the live file's notice to Design.
...
*[The generated block that stood here - `D18Inventory.Emit`'s, between its own markers - stays in `CLAUDE_DESIGN_ASSET_REQUEST.md` under THE GENERATED ASSET INVENTORY: `D18Inventory` writes and reads it there, and a key does not move when its section does.]*
