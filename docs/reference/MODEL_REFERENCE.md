# PoliSim - the model reference

> Moved out of `CLAUDE.md` on 2026-09-22 (`COMPLETED.md` §579), VERBATIM: these sections describe HOW THE MODEL WORKS and are searched by name, never
> auto-loaded. `CLAUDE.md` was 1.39 MB and was read into every session before its own START HERE could say not to read it. Nothing here changed but its
> address: every heading, every figure and every § pointer is as it was. What is a RULE a session must obey stayed in `CLAUDE.md`; what is an explanation is here.
...
- **Starting portfolios** (`WorldFactory.SeedTaxLines`): each country's `IncomeTax`, `CorporateTax`,
  ...
  (no carbon tax distinct from the ETS). Its revenue is rate × the taxed tonnes (`TaxBases.RevenueAtRate`: TRANSPORT
  CO₂ per head × population since EN-4d, ruled 2026-09-11 and built 2026-09-12, §467 - the ETS-covered power fleet is
  exempt of the national carbon tax by statute so the two prices do not stack, applied at SECTOR level for want of an
  installation register, the deviation stated in `EnergyMarket`'s class doc; into the book's dollars by the ECB rate),
  the dispatch does NOT read the tax (the fleet's carbon price is the ETS, exogenous, stepped only by a probe), and the
...
### Quirks recorded (first two 2026-09-05, P5 item 4 of the standing sheet; added since as they were found)
...
- **A film hangs when the Editor loses focus - and a MINIMIZED Editor is the limit of the fix.** `ProjectSettings` carries `runInBackground: 0`, and with it off play mode stops ticking the moment another window takes the foreground: three France films hung at the Budget screen on 2026-09-10 while Chrome was in front - the log trickling one `FRAME:` line a minute, Unity at ~0.2 s of CPU per 10 s, never reaching `entering WaitForEndOfFrame` - where the unattended overnight films had run through. `UiScreenshotCapture.AttachDriver` now forces `Application.runInBackground = true` for the film's own play session (logged `SHOT: runInBackground forced on`; the shipping PlayerSettings value is untouched), and films run through with another app focused. ⚠ **The stated limit: a minimized Editor draws no Game View at all, so a minimized film still hangs** - and restoring the window afterwards did not wake the hung run. Diagnose a quiet film by Unity's CPU and whether its window is iconic before blaming the harness or the screen it stopped on. §431.
- **PowerShell reads `$ds?geo=` as a variable named `ds?`.** A URL built as `"$base/$ds?geo=SE"` sends `…/data/geo=SE` and every dataset answers 404; write `${ds}?geo=`. Found on the first Eurostat probe of the energy sourcing (§455), when ten datasets that had answered the day before all "vanished". Likewise `@(@('url','name'))` is a two-string array, not a one-item list of pairs - `foreach ($p in …) { $p[1] }` then indexes characters (§458).
- **A new generated catalog needs a placeholder for its first generator run.** The runtime and the checks that read `Generated.X` are compiled before the generator can write it, so the first run fails to compile; a hand-written placeholder with the members' names and empty arrays lets the assembly build, the generator overwrites it whole, and nothing hand-written survives (§457, §458). The ITANES catalog met the same wall the other way (the check reverted for the run, §442).
- **A merit order over one plant per category is winner-take-all.** In an annual block the dependable capacity of a whole fleet exceeds the block's residual, so the cheapest category serves everything and no calibration adder can reach an interior share - the first EN-3 calibration produced the USA all gas and Germany all gas with the adders tied to the cent. A fleet is not one plant: offered in tranches (200, ± 25 % around the mean cost) the share becomes a near-continuous function of the adder and the seed calibrates to the thousandth (§459).
- **A tax line's points are the country's currency; a market's costs need not be.** The carbon tax's point reads as one unit of the COUNTRY's currency per tonne (`EnergyMarket.CarbonTaxPointPerTonne`, a stated convention) while the merit order's costs are in euro for the five (dollars for the USA), and EN-3's first landing added zloty points to euro costs - Poland's "twenty points" priced as €20 where it is €4.40. Found by arming the out-of-sample response as a standing check (§461): a reference read off the model is only as good as the units it was read in. The bridge is the ECB reference rate in the catalog (`NationalPerMarketCurrency`). And the BOOK is in a third unit: the game keeps every country's book in US dollars (Sweden's GDP seeds at 620, the USA's at 29 000; the Swedish and Polish budget lines are share weights of a dollar total) - the fiscal layer's first ledger booked Swedish bills in kronor against a budget line in dollars and read industry's electricity at 15 % of GDP; `UsdPerMarketCurrency` takes the stack into the book's dollars, and the plate says USD per kWh. A `Generated.X` name inside `PoliSim.UI` needs the full `PoliSim.Data.Generated.` qualification, as it does inside `PoliSim.EditorTools`.
- **A static per-turn value leaks into the next world built in the same process, and the symptom is every lever reading LIVE.** `EnergyMarket` keeps the turn's water value in a static (set by `BeginTurn`, meant for the turn); the simulation bar builds dozens of worlds in one process, and EN-4's seed fit for Sweden read the PREVIOUS world's last turn value - so two worlds built in a row differed at the seed, and `LeverLivenessCheck` reported nine retired levers (the SWF dial nothing reads among them) as moving the model: the baseline and the treated world were never the same world. Caught by the sim bar's first run (§463), invisible on the cheap bar. The rule: a seed fit reads the seed's own inputs explicitly (`ClearAtSeed`), and `WorldFactory.CreateDefault` resets every static turn value before the families seed (`ResetTurnState`). When a liveness or parity check reports a lever that reaches nothing as live, suspect a static before the lever.
- **A probe on an exogenous price is set AFTER the world is built and cleared in `finally`.** `WorldFactory.CreateDefault` calls `EnergyMarket.ResetTurnState`, which zeroes every probe (the links' scale, the inflow, the ETS rise and its scope), so a diagnostic that sets `ProbeEtsRisePerT` before building its world steps nothing; and a probe that stands at the top of year 1's turn moves everything `BeginTurn` reads in the SAME year - Sweden's water value with Germany's fleet - where a tax decision applied after the top of the turn reached it a year later (§465's "a year behind" was the decision's placement, §467). A per-country scope (`ProbeEtsRiseOnly`) isolates one fleet; the game never sets either.
- **A statistics table's two columns are not always two years.** The BLS relative-importance table for December 2023 prints two figures per row - "Electricity 2.428 2.890" - and the first read took them for December 2022 and 2023; the header says CPI-U and CPI-W. The USA's electricity weight is 2.428 (CPI-U). Read the header before the row, and record which column a figure came from (§465).
