...
> **SOURCED 2026-09-10, `COMPLETED.md` §455, under the ruling of `POLISIM_ENERGY_SPECLET.md` (S1–S15, §454). BUILT the same night, §457:** `Tools/energy_prep.pl` derives `EnergyData/` from the files below, `EnergyCatalogGenerator` emits `EnergyLayerData`, `EnergyLayer` reads it, `EnergyLayerCheck` holds the five gates of §11 on the bar, `EnergyLayerDump` writes `ENERGY_LAYER_PREMISE.md`. The build refined §9's method: the derived part is the MAIN-ACTIVITY producers' electricity (EDGAR's 1A1 is main activity; autoproducers are industry's book) - the reading in §9, which lumped all producers, is superseded by the dump's table. Every figure below is read off a file in `PoliSim-captures/sources/energy/` whose digest is in §12, or off a page fetched this night and saved beside it. **No figure from the concept document or from Design's prototype is in this spine.** Where two independent sources exist the gate of §342 is applied and its result printed - passed, or not passed with the reason. Where a series could not be reached the row says BILLED and names it.
